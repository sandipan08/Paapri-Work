...
define(['N/search', 'N/record','N/format','N/runtime'], function (search, record,format, runtime) {
    ...
    function _get(context) {
        let peerCheck = context.peerCheck
        ...
        if (employeeDetailsObj.isSuccess) {
            if (peerCheck != 1) {
                ...
                let pmcTransactionId = context.pmcTransactionId;

                let pmcRecordObj = record.load({
                    type: 'customrecord_pct_pmc_tran',
                    id: pmcTransactionId,
                    isDynamic: true
                })
                let prevPeerCheckDetails = pmcRecordObj.getValue({
                    fieldId: 'custrecord_pct_pmc_peer_check_details'
                })
                let peerCheckDetails = '';
                if(prevPeerCheckDetails.length > 0){
                    //\r\n
                    peerCheckDetails = prevPeerCheckDetails + '\r\n'+employeeDetailsObj.data.employeeName  +' - '+ currentTime()
                }else{
                    peerCheckDetails = employeeDetailsObj.data.employeeName  +' - '+ currentTime()
                }
                pmcRecordObj.setValue({
                    fieldId: 'custrecord_pct_pmc_peer_check_details',
                    value : peerCheckDetails
                }).save()
            }
        }
        ...
        var entitygroupSearchObj = search.create({
            ...
            filters:
                [
                    ...
                    "AND",
                    ["groupname", "doesnotstartwith", "OSP"]
                ],
            columns:
                [
                    search.createColumn({
