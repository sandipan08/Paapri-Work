...
define(['N/record', 'N/search', 'N/format'], function (record, search, format) {
    ...
    function _post(context) {
        try {
            ...
            const productionCustomLot = context.productionCustomLot
            ...
            if (workOrderId) {
                ...
                if (workOrderDetailsObj.isSuccess) {
                    ...
                    workOrderCompletionObj['productionCustomLot'] = productionCustomLot
                    log.debug({
                        title: 'PCT-PMC',
                    ...
                    if (itemObj.isSuccess) {
                        ...
                        workOrderCompletionObj['usebins'] = itemObj.data.usebins ? true : false;
    ...
    const createAssetRecord = (assetArray, woCompletion, workOrderId) => {
        log.debug({
            title: 'assetArray',
            details: assetArray
        })
        assetArray.forEach(asset => {
            if (asset.lotNumber != '') {
                let assetRecord = record.create({
                    ...
                    fieldId: 'custrecord_epc_wo_comp',
                    value: woCompletion
                }).save()
                /*.setValue({
                    fieldId: 'custrecord_epc_item_asset',
    ...
    const createWorkOrderCompletion = (dataObj) => {
        ...
        let productionCustomLot = dataObj.productionCustomLot
        ...
        let assembly = workOrderCompletionRecord.getValue({
            fieldId: 'item',
        })
        if (dataObj.isLastOperation && (dataObj.isLotItem || dataObj.usebins) && dataObj.completedQuantity > 0) {
            let subRecord = workOrderCompletionRecord.getSubrecord({
                fieldId: 'inventorydetail'
            })
            if (dataObj.isLotItem) {
        ...
        else if (dataObj.isLastOperation && (dataObj.isserialitem || dataObj.usebins) && dataObj.completedQuantity > 0) {
        ...
        let assetArray = [{ lotNumber: productionCustomLot, component: assembly }]

        createAssetRecord(assetArray, workOrderCompletionId, dataObj.workOrderId)
    ...
    const checkIsLotNumberedItem = (itemId) => {
        ...
        if (searchResultCount > 0) {
            ...
            itemSearchObj.run().each(function (result) {
                ...
                itemDataObj['usebins'] = result.getValue({ name: "usebins" })
                // .run().each has a limit of 4,000 results
                return true;
            });
