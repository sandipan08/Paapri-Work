...
define(['N/record', 'N/search'], function (record, search) {
    ...
    function _get(context) {
        try {
            ...
            let workOrderItemDetails = getWorkOrderItemDetails(workOrderId, operationSequence);
            ...
            let qualityALert = checkQualityAlert(itemId)
            ...
            return { 'isSuccess': true, 'workOrderItemDetails': workOrderItemDetails, 'workOrderOperationTaskDetails': workOrderOperationTaskDetails, 'toolFamilyDetails': toolFamilyTaskDetails, 'pmcTransactionDetails': pmcTransactionDetails, 'serializedToolDetails': serializedToolTaskDetails, 'PMCworkInstructionDetails': PMCworkInstructionDetails, 'workInstructionDetails': workInstructionDetails, 'processInputDetails': processInputsDetails, 'isLastOperation': isLastOperation.isSuccess, qualityALert: qualityALert }
        }
        catch (error) {
            return { 'isSuccess': false, 'errorMessage': error.message }
        }
    }
    const checkQualityAlert = (itemId) => {
        let qualityALert = ''
        var customrecord_pct_qms_quality_alertSearchObj = search.create({
            type: "customrecord_pct_qms_quality_alert",
            filters:
                [
                    ["custrecord_pct_qms_qa_item_no", "anyof", itemId],
                    "AND",
                    ["isinactive", "is", "F"],
                    "AND",
                    ["custrecord_pct_qms_qa_removal_date", "onorafter", "today"]
                ],
            columns:
                [
                    search.createColumn({ name: "name", label: "ID" }),
                    search.createColumn({ name: "scriptid", label: "Script ID" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_issues_to", label: "Issued To" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_case_no", label: "Case #" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_customer", label: "Customer" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_item_no", label: "Item #" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_type", label: "Type" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_picture_1", label: "Picture1" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_picture_2", label: "Picture2" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_car_no", label: "CAR#" }),
                    search.createColumn({ name: "custrecord_pct_qms_qa_aler_detail", label: "Alert Detail" })
                ]
        });
        var searchResultCount = customrecord_pct_qms_quality_alertSearchObj.runPaged().count;
        log.debug("customrecord_pct_qms_quality_alertSearchObj result count", searchResultCount);
        customrecord_pct_qms_quality_alertSearchObj.run().each(function (result) {
            qualityALert = result.getValue('custrecord_pct_qms_qa_aler_detail')
            // return true;
        });
    ...
    const getWorkOrderItemDetails = (workOrderId, operationSequence) => {
        let keyObjects = {
            'itemText': '',
            'itemDesc': '',
            'itemId': '',
        ...
        try {
            ...
            let routingId = workOrderRecord.getValue({
            ...
            let componentPerOperation = getComponentPerOperation(routingId, operationSequence)
            ...
            if (itemLineCount > 0) {
                ...
                for (let lineIndex = 0; lineIndex < itemLineCount; lineIndex++) {
                    ...
                    let itemId = workOrderRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'item',
                        line: lineIndex
                    })
                    let operation = workOrderRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'operationdisplaytext',
                        line: lineIndex
                    })
                    // log.debug("operation",operation)
                    let assetRequired = search.lookupFields({
                        type: 'item',
                        id: itemId,
                        columns: ['custitem_pct_epc_asset_required']
                    });
                    if (operation == operationSequence ) {
                        log.debug("HEREEEE",assetRequired.custitem_pct_epc_asset_required)
                        if (componentPerOperation.indexOf(itemId) != -1 && assetRequired.custitem_pct_epc_asset_required) {
log.debug("HEREEEE 2")
                            ...
                            res.woIssueModalCustomLot = `<input type="text"
                                        class="form-control assetValue" value="">`
    ...
    const getComponentPerOperation = (routingId, operationSequence) => {

        let operationWiseItem = []
        let revObj = record.load({
            type: record.Type.MANUFACTURING_ROUTING,
            id: routingId,
            isDynamic: true
        })

        let lineCount = revObj.getLineCount({
            sublistId: 'routingcomponent'
        })
        log.debug({
            title: 'routingId' + ' operationSequence =' + operationSequence,
            details: routingId + 'lineCount =' + lineCount
        })
        for (let index = 0; index < lineCount; index++) {
            ...
            let opSeq = revObj.getCurrentSublistValue({
                sublistId: 'routingcomponent',
            ...
            //         details: opSeq
            //     })
            if (opSeq == operationSequence) {

                operationWiseItem.push(revObj.getCurrentSublistText({
                    sublistId: 'routingcomponent',
                    fieldId: 'item'
                }))
            }
        ...
        log.debug({
            title: 'operationWiseItem',
            details: operationWiseItem
        })
        return operationWiseItem
    }
    ...
    const getTableHeaders = () => {
        return {
            'itemsTableTitleArr': ['Item', 'Description', 'Item Id', 'Quantity', 'Total Qty Issued', 'Remaining Qty', 'Back Ordered', 'Component Yield', 'Unit of Measure', 'Line No', 'Asset #',],
