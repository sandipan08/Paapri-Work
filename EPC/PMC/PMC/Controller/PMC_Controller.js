...
let restletsScriptObj = {
    ...
    'customscriptcustomscript_pct_pmc_get_com': {
        'scriptName': 'PCT PMC Get Component for Operation',
        'scriptId': 'customscriptcustomscript_pct_pmc_get_com',
        'scriptInternalId': ''
    },
    'customscript_pct_pmc_get_bin': {
        'scriptName': 'PCT PMC Get Bin Number from Work Center',
        'scriptId': 'customscript_pct_pmc_get_bin',
        'scriptInternalId': ''
    },
    'customscript_pct_epc_get_components': {
        'scriptName': 'PCT EPC Get Work Order Components',
        'scriptId': 'customscript_pct_epc_get_components',
        'scriptInternalId': ''
    },
    'customscript_pct_pmc_create_ncmr_record': {
        'scriptName': 'PCT PMC Create NCMR Record',
        'scriptId': 'customscript_pct_pmc_create_ncmr_record',
        'scriptInternalId': ''
    }
}
...
$(document).ready(async () => {
    ...
    $('#logIn').click(async (e) => {
        ...
        if (employeeDetailsObj.responseSuccess && employeeDetailsObj.responseData.isSuccess) {
            ...
            employeeInternalId = loggedInUserDetails.internalId
    ...
    /* --------------------------------------------------- Peer Check Function Start --------------------------------------------------------- */
    $('#peerCheck').on('click', async (e) => {
        if (selectedRowData.pmcTransactionId) {
            Swal.fire({
                title: 'Peer Check',
                html: `<div><input type="number" value="" class="swal2-input" id="peerCheckBadge" min='0' step='1' onkeydown="if(event.key==='.'){event.preventDefault();}" ></div>`,
                width: 700,
                confirmButtonText: 'Ok',
                focusConfirm: false,
                preConfirm: () => {
                    const peerCheckBadge = Swal.getPopup().querySelector('#peerCheckBadge').value


                    if (!peerCheckBadge) {
                        Swal.showValidationMessage(`Please enter Badge Id`)
                    }
                    else if (peerCheckBadge == badgeId) {
                        Swal.showValidationMessage(`Please enter another Badge Id`)
                    }
                    return { peerCheckBadge: peerCheckBadge }
                }
            }).then(async (result) => {
                if (result.isConfirmed) {

                    let peerCheckBadge = result.value.peerCheckBadge


                    let peerEmployeeDetailsObj = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_pmc_badgeid_verify.scriptInternalId}&deploy=1&badgeId=${peerCheckBadge}&peerCheck=1&pmcTransactionId=${selectedRowData.pmcTransactionId}`)
                    if (peerEmployeeDetailsObj.responseSuccess && peerEmployeeDetailsObj.responseData.isSuccess) {
                        isUserLoggedIn = true;

                        let peerLoggedInUserDetails = peerEmployeeDetailsObj.responseData.data;
                        let peerEmployeeName = peerLoggedInUserDetails.employeeName
                        generateAlert({
                            'type': 'success',
                            'title': `Peer check-in was successfully completed by ${peerEmployeeName}`,
                            'message': '',
                            'timer': 10000,
                            'showConfirmButton': false,
                            'timerProgressBar': true
                        ...
                        generateError({ 'errorMessage': 'Peer Badge Id Not found' })
                        enableUI();
                    }
                }
                else {
            ...
            generateError({ 'errorMessage': 'Kindly start the job first.' })
        }
    })
    /* --------------------------------------------------- Peer Check Function End --------------------------------------------------------- */


    /* --------------------------------------------------- Create NCMR Fucntion Start --------------------------------------------------------- */
    $('#createNCMR').on('click', async (e) => {
  ...
  <div class="form-container">
    ...
    <form>
      <div class="form-group">
        ...
        <option value="-1">-- Please choose a component --</option>`;
        let componentResponseObj = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_epc_get_components.scriptInternalId}&deploy=1&workOrderId=${selectedRowData.workOrderId}`)
        if (componentResponseObj.responseSuccess && componentResponseObj.responseData.isSuccess) {
            console.log(componentResponseObj.responseData.data)
            componentResponseObj.responseData.data.map((element) => {

                formHtml += `<option value="${element.internalId}">${element.name}</option>`;
            })
        }
        else {
            generateError({ 'errorMessage': componentResponseObj.responseData.errorMessage })
        }
        enableUI();
        formHtml += `  </select>
      </div>
      ...
      <div class="form-group">
        <label for="assetNumbers">Asset Numbers</label>
        <input type="text" id="assetNumbers" name="assetNumbers" placeholder="Asset Number(s)">
      </div>
...
`;
        Swal.fire({
            ...
            focusConfirm: false,
            preConfirm: () => {
                const component = Swal.getPopup().querySelector('#component').value
                const quantity = Swal.getPopup().querySelector('#quantity').value
                const assetNumbers = Swal.getPopup().querySelector('#assetNumbers').value
                const notes = Swal.getPopup().querySelector('#notes').value
                if (!component) {
                    Swal.showValidationMessage(`Please select the component`)
                }
                else if (!quantity) {
                    Swal.showValidationMessage(`Please enter the input quantity`)
                }
                else if (!assetNumbers) {
                    Swal.showValidationMessage(`Please enter the asset numbers`)
                }

                return { component: component, quantity: quantity, assetNumbers: assetNumbers }
            }
        }).then(async (result) => {
            if (result.isConfirmed) {
                let ncmrObj = {
                    ...
                    'assetNumbers': result.value.assetNumbers,
                    'notes': result.value.notes,
                    'workOrder': selectedRowData.workOrderId,
                    'operationSequence': selectedRowData.operationSequence,
                    'employeeId': loggedInUserDetails.internalId
                }
                console.log(ncmrObj)
                const ncmrResponseObj = await fetchPostRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_pmc_create_ncmr_record.scriptInternalId}&deploy=1`, ncmrObj)
                if (ncmrResponseObj.responseSuccess && ncmrResponseObj.responseData.isSuccess) {
                    generateAlert({
                        'type': 'success',
                        'title': `NCMR Record Created. NCMR Record Number is ${ncmrResponseObj.responseData.data.name}`,
                        'message': '',
                        'timer': 1000,
                        'showConfirmButton': false,
                        'timerProgressBar': true
                    ...
                    generateError({ 'errorMessage': ncmrResponseObj.responseData.errorMessage })
                    enableUI();
                }
            }
            else {
    ...
    $('#production').on('click', async (e) => {
        ...
        if (!($(e.target).hasClass('active'))) {
            // let woIssueCreateResponseObj = await fetchPostRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscriptcustomscript_pct_pmc_get_com.scriptInternalId}&deploy=1`, { 'workOrderId': selectedRowData.workOrderId, 'operationTaskId': selectedRowData.operationSequence });
    ...
    $("#workOrderIssue").click(async () => {
        ...
        // workOderIssueTableData.columnsArr.push({ 'title': 'Lot', 'defaultContent': '<input type="text" class="form-control issueCustomLot" style="width:200px" value="">' });
        ...
        workOrderIssueTable = $('#workOrderIssueTable').DataTable({
            ...
            "pageLength": 100,
            columnDefs: [
                { 'visible': false, 'targets': [2, 7, 9] },],
            initComplete: function (settings, json) {
                $("#workOrderIssueTable").wrap("<div style='overflow:auto; width:100%;position:relative;'></div>");
            }, "fnRowCallback": function (nRow, aData, iDisplayIndex) {
                // console.log(JSON.stringify(aData))
                let remainingQty = parseFloat(aData.quantity) - parseFloat(aData.usedInBuild)
                $(nRow).closest('tr').find('.issueQuantity').val(remainingQty)
            }
        });
        ...
        $('.inventoryDetail').on('click', async (e) => {
            ...
            let getBinNumberResponse = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_pmc_get_bin.scriptInternalId}&deploy=1&workCenterId=${selectedRowData.workCenterId}`)
            // if (peerEmployeeDetailsObj.responseSuccess && peerEmployeeDetailsObj.responseData.isSuccess) {

            enableUI()
            console.log(getBinNumberResponse)
            ...
            let assetValue = $(currentRow).find('.assetValue').val();
            // let issueCustomLot = parseFloat($(currentRow).find('.issueCustomLot').val());
            ...
            if (issueQuantity > 0) {
                ...
                if (!(itemInternalID in workOrderIssueTableDataObj)) {
                    if (!(selectedRowData.manufacturingOperationTaskId in woIssueData)) {
                        ...
                        woIssueData[selectedRowData.manufacturingOperationTaskId][primaryKey]['assetValue'] = assetValue
                        // woIssueData[selectedRowData.manufacturingOperationTaskId][primaryKey]['issueCustomLot'] = issueCustomLot
                        woIssueData[selectedRowData.manufacturingOperationTaskId][primaryKey].modalData = [{
                            lotNumber: "",
                            binNumber: getBinNumberResponse.responseData.data.stagingbin,
        ...
        //                 $('.woIssueModalCommitLineQuantity').val(issueQuantity)
        ...
        //                     if (loggedInUserDetails.location == PowayCALocation) {
        //                         defaultBinId = 2
        //                     }
        //                     else if (loggedInUserDetails.location == GreenvilleSCLocation) {
        //                         defaultBinId = 702
        //                     }
        ...
        //                         $('.binDropdownTd').html(binSelectField)
        //                         $('#woIssueModalBinSelect').val(defaultBinId)
        //                     }
        //                     else {
    ...
    $("#quality").on("click", async () => {
        ...
        let qualityDataResponse = await fetchGetRequest(
            `/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_pmc_draw_spec_data_get
                .scriptInternalId
            }&deploy=1&params=${JSON.stringify(params)}`
        );
        console.log("qualityDataResponse: ", qualityDataResponse)
        if (
            qualityDataResponse.responseSuccess &&
            qualityDataResponse.responseData.isSuccess
        ) {
            qualityData = qualityDataResponse.responseData;
            //Drawing Specification Table
            let drawingSpecificationThead = [
                //  "Sl No.",
                // "Version",
                "Feature",
                "UOM",
            ...
            let drawingSpecificationTbody =
                qualityDataResponse.responseData.drawingSpecificationData;
            console.log(drawingSpecificationTbody)
            let drawingTable = generateTable(
                drawingSpecificationThead,
                drawingSpecificationTbody
            ).data;
            ...
            if (!qualityDataResponse.responseData.isSavedData) {
                measurementTbody = Array(
                    qualityDataResponse.responseData.drawingSpecificationData.length
                ).fill(qualityDataResponse.responseData.measurementData);
            }
            ...
            $("#measurmentTable tbody tr").each((index, element) => {
                ...
                highValue.toLowerCase() === "accept"
                    ? $(element).find(".high").css("background", greenColourCode)
                    : highValue.toLowerCase() === "reject"
                        ? $(element).find(".high").css("background", redColourCode)
                        : highValue.toLowerCase() === "alert"
                            ? $(element).find(".high").css("background", yellowCOlourCode)
                            : $(element).find(".high").css("background", "none");
                lowValue.toLowerCase() === "accept"
                    ? $(element).find(".low").css("background", greenColourCode)
                    : lowValue.toLowerCase() === "reject"
                        ? $(element).find(".low").css("background", redColourCode)
                        : lowValue.toLowerCase() === "alert"
                            ? $(element).find(".low").css("background", yellowCOlourCode)
                            : $(element).find(".low").css("background", "none");
            });

            //Inspection Table
            let inspectionResponse = qualityDataResponse.responseData.additionalData;
            let inspectionTbodyResponse =
                qualityDataResponse.responseData.inspectionData;

            var maxHeader = 0;

            for (var i = 0; i < drawingSpecificationTbody.length; i++) {
                var noOfTesting = drawingSpecificationTbody[i].noOfTesting.data;
                if (noOfTesting > maxHeader) {
                    maxHeader = noOfTesting;
                }
            }
            ...
            for (
                let inspectionHeaderIndex = 0;
                inspectionHeaderIndex < maxHeader;
                inspectionHeaderIndex++
            ) {
            ...
            for (
                let inspectionHeaderIndex = 0;
                inspectionHeaderIndex < drawingSpecificationTbody.length;
                inspectionHeaderIndex++
            ) {
                inspectionTbody += `<tr>`;
                var noOfTesting =
                    drawingSpecificationTbody[inspectionHeaderIndex].noOfTesting.data;
                var max = 0;
                if (noOfTesting > max) {
                    max = noOfTesting;
                }
                ...
                for (let sampleIndex = 0; sampleIndex < max; sampleIndex++) {
                    if (sampleIndex < noOfTesting) {
                        inspectionTbody += qualityDataResponse.responseData.isSavedData
                            ? qualityDataResponse.responseData.additionalData.submitted
                                ? `<td><input type="number" class="form-control inspectionTextBox" value='${inspectionTbodyResponse[inspectionHeaderIndex][sampleIndex]}' disabled></td>`
                                : `<td><input type="number" class="form-control inspectionTextBox" value='${inspectionTbodyResponse[inspectionHeaderIndex][sampleIndex]}'></td>`
                            : `<td><input type="number" class="form-control inspectionTextBox" value=''></td>`;
                    }
                    // else {
                    //     inspectionTbody += qualityDataResponse.responseData.isSavedData ? qualityDataResponse.responseData.additionalData.submitted ? `<td><input type="number" class="form-control inspectionTextBox" value='${inspectionTbodyResponse[inspectionHeaderIndex][sampleIndex]}' disabled></td>` : `<td><input type="number" class="form-control inspectionTextBox" value='${inspectionTbodyResponse[inspectionHeaderIndex][sampleIndex]}' disabled></td>` : `<td><input type="number" class="form-control inspectionTextBox" value='' disabled></td>`;
                    // }
    ...
    $(document).on("input", ".inspectionTextBox", (e) => {
        ...
        let rowMean = rowSum / rowData.length; //parseFloat(qualityData.additionalData.sampleSize)
        ...
        let nominal = parseFloat(
            $(`#drawingSpecificationTable tbody tr:eq(${currentRowIndex})`)
                .find(".nominal")
                .val()
        );
        let positiveTolerance = parseFloat(
            $(`#drawingSpecificationTable tbody tr:eq(${currentRowIndex})`)
                .find(".positiveTol")
                .val()
        );
        let negetiveTolerance = parseFloat(
            $(`#drawingSpecificationTable tbody tr:eq(${currentRowIndex})`)
                .find(".negativeTol")
                .val()
        );
        let failureAlert = $(`#drawingSpecificationTable tbody tr:eq(${currentRowIndex})`)
            .find(".failureAlert")
            .val()
        // let upperValue =
        //   rowData[0] === ""
        ...
        //     : positiveTolerance === 0
        //     ? "-"
        //     : (rowMaxValue - nominal) / positiveTolerance < 0
        //     ? 0
        //     : (rowMaxValue - nominal) / positiveTolerance;
        // let lowerValue =
        //   rowData[0] === ""
        //     ? ""
        //     : negetiveTolerance === 0
        //     ? "-"
        //     : (nominal - rowMinValue) / negetiveTolerance < 0
        //     ? 0
        //     : (nominal - rowMinValue) / negetiveTolerance;
        ...
        let upperValue =
            rowData[0] === ""
                ...
                : positiveTolerance === 0
                    ? "-"
                    : ((rowMaxValue - nominal) / nominal) * 100 < 0
                        ? 0
                        : ((rowMaxValue - nominal) / nominal) * 100;
        ...
        let lowerValue =
            rowData[0] === ""
                ...
                : negetiveTolerance === 0
                    ? "-"
                    : ((nominal - rowMinValue) / nominal) * 100 < 0
                        ? 0
                        : ((nominal - rowMinValue) / nominal) * 100;
        ...
        //     ? "Reject"
        //     : positiveTolerance === 0 &&
        //       rowMaxValue > nominal - 0.175 * negetiveTolerance
        //     ? "Alert"
        //     : upperValue === "-"
        //     ? "Accept"
        //     : upperValue > 0.65
        ...
        //   lowerValue === ""
        //     ? ""
        //     : rowMinValue < nominal - negetiveTolerance
        //     ? "Reject"
        //     : negetiveTolerance === 0 &&
        //       rowMinValue < nominal + 0.175 * positiveTolerance
        //     ? "Alert"
        //     : lowerValue === "-"
        //     ? "Accept"
        //     : lowerValue < 0.65
        ...
        $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
            .find(".mean")
            .val(rowMean.toFixed(3));
        $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
            .find(".upper")
            .val(
                !isNaN(parseFloat(upperValue))
                    ? parseFloat(upperValue).toFixed(3)
                    : upperValue
            );
        $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
            .find(".lower")
            .val(
                !isNaN(parseFloat(lowerValue))
                    ? parseFloat(lowerValue).toFixed(3)
                    : lowerValue
            );
        $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
            .find(".high")
        ...
        highValue.toLowerCase() === "accept"
            ? $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
                .find(".high")
                .css("background", greenColourCode)
            : highValue.toLowerCase() === "reject"
                ? $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
                    .find(".high")
                    .css("background", redColourCode)
                : highValue.toLowerCase() === "alert"
                    ? $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
                        .find(".high")
                        .css("background", yellowCOlourCode)
                    : $(`#measurmentTable tbody tr:eq(${currentRowIndex})`)
                        .find(".high")
                        .css("background", "none");
        lowValue.toLowerCase() === "accept"
        ...
        //     type: "warning",
        //     title: "Warning",
        //     message: lowValue.toLowerCase() != "accept" ? "Lower value is rejected" : highValue.toLowerCase() != "accept" && "Higher value rejected",
        //     timer: 1500,
        //     showConfirmButton: false,
        //     timerProgressBar: true,
        // });
    ...
    // Modify on 6/12/25
    $(document).on("blur", ".inspectionTextBox", (e) => {

        console.log(failAlert)
        if (lowerVal.toLowerCase() != "accept") {
            Swal.fire({
                title: "Lower value is rejected",
                text: `${failAlert}`,
                icon: "warning",
            ...
            //     type: "warning",
            //     title: "Warning",
            //     message: "Lower value is rejected",
            //     timer: 1500,
            //     showConfirmButton: false,
            //     timerProgressBar: true,
            // });
        } else if (upperVal.toLowerCase() != "accept") {
            // generateAlert({
            //     type: "warning",
            //     title: "Warning",
            //     message: "Upper value is rejected",
            //     timer: 1500,
            //     showConfirmButton: false,
            //     timerProgressBar: true,
            // });
            Swal.fire({
                title: "Upper value is rejected",
                text: `${failAlert}`,
                icon: "warning",
                allowOutsideClick: false,
                allowEscapeKey: false,
        ...
        } else if (upperVal.toLowerCase() != "accept" && lowerVal.toLowerCase() != "accept") {
            Swal.fire({
                title: "Upper and lower both values are rejected",
                text: `${failAlert}`,
                icon: "warning",
                allowOutsideClick: false,
                allowEscapeKey: false,
            ...
            //     type: "warning",
            //     title: "Warning",
            //     message: "Upper and lower values are rejected",
            //     timer: 1500,
            //     showConfirmButton: false,
            //     timerProgressBar: true,
            // });
    ...
    $("#qualitySubmit").click(() => {
        let selectedRowOperationSequence = selectedRowData.operationSequence
        createOrUpdateQuality({ toBeSaved: false, selectedRowOperationSequence: selectedRowOperationSequence, employeeInternalId: employeeInternalId });
    });
    ...
    $("#qualitySave").click(() => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let selectedRowOperationSequence = selectedRowData.operationSequence
                createOrUpdateQuality({ toBeSaved: true, selectedRowOperationSequence: selectedRowOperationSequence, employeeInternalId: employeeInternalId });
                $("#qualitySubmit").attr("disabled", true);
                $("#qualityCancel").attr("disabled", true);
                $("#qualitySave").attr("disabled", true);
    ...
    const createOrUpdateQuality = async (parameters) => {
        ...
        for (
            ...
            dataCountIndex < qualityData.drawingSpecificationData.length;
            dataCountIndex++
        ) {
            let qualityDataObj = {};
            //   let selectedRowOperationSequence =  selectedRowData.operationSequence
            //   qualityDataObj["operationSequence"] = selectedRowOperationSequence;

            qualityDataObj["inspectionRecordId"] =
                qualityData.additionalData.inspectionRecordId;
            qualityDataObj["sampleSize"] = qualityData.additionalData.sampleSize;
            // -------------------- Operation for Drawing Specification Data ----------------------------
            let drawingTableDataArray = [],
                testResult = [];
            $(
                `#drawingSpecificationTable tbody tr:eq(${dataCountIndex}) td input`
            ).each((key, element) => {
            ...
            $(`#measurmentTable tbody tr:eq(${dataCountIndex}) td input`).each(
                (key, element) => {
                    ...
                    else measurementObject["mean"] = $(element).val();
                }
            );
            qualityDataObj["measurmentResults"] = measurementObject;
            // -------------------- Operation for Inspection Record Data ----------------------------
            $(`#inspectionTable tbody tr:eq(${dataCountIndex}) td input`).each(
                (key, element) => {
                    testResult.push($(element).val());
                }
            );
            qualityDataObj["inspectionRecord"] = testResult.join(";");
            let drawingSpecificationChildResponse =
                qualityData.drawingSpecificationChildId;
            qualityDataObj["internalId"] = qualityData.isSavedData
                ? drawingSpecificationChildResponse[dataCountIndex].internalId
                : "";
            qualityDataObj["toBesaved"] = parameters.toBeSaved;
            let selectedRowOperationSequence = selectedRowData.operationSequence
            qualityDataObj["selectedRowOperationSequence"] = selectedRowOperationSequence;
            qualityDataObj["employeeInternalId"] = loggedInUserDetails.employeeName
            // qualityDataObj["employeeName"] =  employeeInternalId
            qualityDataArray.push(qualityDataObj);
        }
        let updateInspectionRecordResponseObj = await fetchPostRequest(
            `/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_pmc_update_inspection_d.scriptInternalId}&deploy=1`,
            qualityDataArray
        );
        if (
            updateInspectionRecordResponseObj.responseSuccess &&
            updateInspectionRecordResponseObj.responseData.isSuccess
        ) {
            generateAlert({
                type: "success",
    ...
    $(document).on('click', '.woIssueModalCommitLineButton', (e) => {
        ...
        if (itemId in workOrderIssueTableDataObj) {
            ...
            let customLot = $(tr).find('.woIssueModalCommitLineCustomLot').val();
    ...
    $(document).on('click', '#woIssueModalSave', (e) => {
        ...
        if (qtyToBeCommited === commitedQuantity) {
            $('#woIssueModal_body_table tbody tr').each((index, element) => {
                ...
                eachRowData['customLotNumber'] = $(element).find('.woIssueModalCommitLineCustomLot').val();
...
const getQualityStatus = async () => {
    let QualityStatusResponse = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=customscript_pct_pmc_check_quality_statu&deploy=customdeploy_pct_pmc_check_quality_statu&workOrderId=${selectedRowData.workOrderId}&operationSequence=${selectedRowData.operationSequence}&operationTaskId=${selectedRowData.manufacturingOperationTaskId}&itemId=${selectedRowData.itemId}&workCenterId=${selectedRowData.workCenterId}`);
    console.log('QualityStatusResponse' + JSON.stringify(QualityStatusResponse));
    if (QualityStatusResponse.responseSuccess && QualityStatusResponse.responseData.QualityPassStatus) {
        return true;
...
const stopTimerClock = async () => {
    ...
    let completionObj = {
        ...
        'productionCustomLot': '',
        'productionEndTime': currentAccountDate
    }
    if (selectedRowData.pmcTransactionName.toLowerCase() === 'production') {
        ...
        let selectedRowWorkOrderId = selectedRowData.workOrderId;
        if (int)
            div = `<div>Quantity <input type="number" oninput="this.value = Math.round(this.value);" value="" class="swal2-input" id="productionQuantity" min='0' step='1'/><span id="swal_uom" style="font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`;
        if (workOrderAdditionalData[selectedRowWorkOrderId].isLastOperation == true) {
            div += `<br/><div>Scrap<input type="number" oninput="this.value = Math.round(this.value);" value="" class="swal2-input" id="productionScrapQuantity" min='0' step='1'><span id="swal_uom" style="font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`;
            div += `<br/><div>Asset #<input type="text" value="" class="swal2-input" id="productionCustomLot" ></div>`;
        }
        clearInterval(int);
        let isQualityPass = await getQualityStatus();
        enableUI();
        // alert(isQualityPass)
        // alert(isQualityPass)
        if (isQualityPass) {
            let productionScrapQuantity = 0
            let productionCustomLot = ''
            Swal.fire({
                title: 'Production Quantity',
                html: div,//`<div><input type="number" value="" class="swal2-input" id="productionQuantity" min='0' step='1' onkeydown="if(event.key==='.'){event.preventDefault();}" ><span id="swal_uom" style="font-size:40px;font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`,
                width: 700,
                confirmButtonText: 'Ok',
                focusConfirm: false,
                preConfirm: () => {
                    ...
                    if (workOrderAdditionalData[selectedRowWorkOrderId].isLastOperation == true) {
                        productionScrapQuantity = Swal.getPopup().querySelector('#productionScrapQuantity').value
                        productionCustomLot = Swal.getPopup().querySelector('#productionCustomLot').value
                    }
                    if (!productionQuantity) {
                        Swal.showValidationMessage(`Please enter Production Quantity`)
                    }
                    return { productionQuantity: productionQuantity, productionScrapQuantity: productionScrapQuantity, productionCustomLot: productionCustomLot }
                }
            }).then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    let woIssueCreateResponseObj = await fetchPostRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscriptcustomscript_pct_pmc_get_com.scriptInternalId}&deploy=1`, { 'workOrderId': selectedRowData.workOrderId, 'operationTaskId': selectedRowData.operationSequence });
                    console.log(woIssueCreateResponseObj)
                    if (woIssueCreateResponseObj.responseSuccess && woIssueCreateResponseObj.responseData.isSuccess) {
                        generateAlert({
                            'type': 'success',
                            'title': `Work Order Issue Created. Work Order Issue Number is ${woIssueCreateResponseObj.responseData.data.name}`,
                    ...
                    enableUI()
                    completionObj.productionScrapQuantity = productionScrapQuantity;
                    completionObj.productionCustomLot = productionCustomLot
                    ...
                    if ((!selectedRowData.predecessorId) && (parseFloat(selectedRowData.inputQuantity) >= parseFloat(completedQty) + parseFloat(qtyEntered))) {
                        completionObj.productionQuantity = qtyEntered;
                    }
                    else {

                        if (parseFloat(selectedRowData.inputQuantity) >= parseFloat(completedQty) + parseFloat(qtyEntered)) {
                            completionObj.productionQuantity = qtyEntered
                        } else {
                            swalAlertObj.type = 'error';
                            swalAlertObj.title = "You cannot process more than the input quantity.";
            ...
            generateError({ 'errorMessage': 'Please Do Quality first!' })
        }
    }
    else {
        let woCompletionResponseObj = await createWorkOrderCompletion(completionObj);
...
const getOpenWorkOrders = async () => {
    ...
    openWorkOrderDataTable = $('#openWorkOrderTable').DataTable({
        ...
        columnDefs: [
            ...
            { 'visible': false, 'targets': [1, 3, 5, 6, 9, 17, 19, 23] },
        ],
        initComplete: function (settings, json) {
    ...
    openWorkOrderDataTable.off('select').on('select', async () => {
        ...
        if (workOrderAdditionalData[selectedRowWorkOrderId].qualityALert != '') {
            //alert(workOrderAdditionalData[selectedRowWorkOrderId].qualityALert)
            generateAlert({
                type: "info",
                title: `${workOrderAdditionalData[selectedRowWorkOrderId].qualityALert}`,
                message: "",
                timer: 10500,
                showConfirmButton: false,
                timerProgressBar: true,
        ...
        $('#pmcworkInstructionTable').DataTable({
            data: workOrderAdditionalData[selectedRowWorkOrderId].PMCworkInstructionDetails.data,
            columns: workOrderAdditionalData[selectedRowWorkOrderId].PMCworkInstructionDetails.columnsArr,
            columnDefs: [
                {
                    targets: workOrderAdditionalData[selectedRowWorkOrderId].PMCworkInstructionDetails.columnsArr.findIndex(element => element.data === 'url'),
                    data: workOrderAdditionalData[selectedRowWorkOrderId].PMCworkInstructionDetails.data,
        ...
        $('#pmcWorkInstructionTableContainer-span').html(workOrderAdditionalData[selectedRowWorkOrderId].PMCworkInstructionDetails.data.length)


        $('#workInstructionTable').DataTable({
            ...
            destroy: true,
            initComplete: function (settings, json) {
                $("#workInstructionTable").wrap("<div style='overflow:auto; width:100%;position:relative;'></div>");
            }
        });
        $('#workInstruction-span').html(workOrderAdditionalData[selectedRowWorkOrderId].workInstructionDetails.data.length)
...
const generateTable = (toolsTableHead, toolsTableBody) => {
    ...
    if (toolsTableHead.length > 0) {
        ...
        toolsTableHead.map((element) => {
            //   table += `<th>${element}</th>`;
            if (element === 'Feature') {
                table += `<th style="width: 300px;">${element}</th>`;
            }
            else if (element === 'Failure Alert') {
                table += `<th style="width: 300px;" hidden>${element}</th>`;
            }
            else {
                table += `<th>${element}</th>`;
            }
    ...
    toolsTableBody.map((element) => {
        ...
        Object.keys(element).map((keys) => {
            ...
            table += `<td><input type=text value ='${element[keys].data.includes('\n') ? element[keys].data?.split('\n')[0]?.split("-")[1]?.trim() : element[keys].data.includes('-') ? element[keys].data.split('-')[1]?.trim() : element[keys].data}' class="form-control ${element[keys].class}" disabled/></td>`;
        })
        table += `</tr>`;
    });
    table += '</tbody>'
