/****************************+**********************************************************
Script Name: UserEvent Action Script 
Developer: Kunal Das
...
****************************************************************************************/
function CreateTool(ToolNumber, SerialID, ItemId, transactionID, BinNumber, nominalId, idUpperLimit, idLowerLimit, nominalOd, odUpperLimit, odLowerLimit, location) {
	...
	rec.setFieldValue('custrecord_pct_mott_tool_location', location);
	//}
	...
	tt_rec.setFieldValue('custrecord_pct_mott_acquisition_transac', transactionID);
...
function CreateToolsIRorIA() {
	...
	var item = record.getFieldValue('item');
	try {
	...
	var subrecord = record.viewSubrecord('inventorydetail');
	// var subrecord = record.viewLineItemSubrecord('inventory', 'inventorydetail', j);
	try {
		...
		for (var k = 1; k <= subrecord.getLineItemCount('inventoryassignment'); k++) {
			...
			if (searchresults != null)
				...
				for (var i = 0; searchresults != null && i < searchresults.length; i++) {
					...
					ret = CreateTool(strSerial, internalId, item, id, binnumber, nominal_id, id_upper_limit, id_lower_limit, nominal_od, od_upper_limit, od_lower_limit, location)
				}
		}
	}
	catch (error) {
