...
define(['N/log', 'N/search', 'N/record'], function (log, search, record) {
    ...
    function _get(context) {
        ...
        workOrderArray.forEach(workOrderId => {
            ...
            Object.keys(estimatedDataObj).forEach(function (key) {
                ...
                var operationWiseActualDataArray = ActualDataFromWOCompletion[workOrderId][key]
                ...
                varianceReportObj[workOrderId][estimatedDataObj[key].operationName].acrCost = totalRunCost + totalSetupCost;
    ...
    const getEstimateDataFromMFGOperationTask = (workOrderArray, costCategoryObj) => {
        ...
        manufacturingoperationtaskSearchObj.run().each(function (result) {
            ...
            operationObj.machineSetUpRate = costObj.totalMachineSetupRate
            operationObj.laborSetUpRate = costObj.totalLaborSetupRate
            operationObj.machineRunRate = costObj.totalMachineRunRate
            operationObj.laborRunRate = costObj.totalLaborRunRate
    ...
    const getCostDataFromMFGOperationTask = (MFGOperationTaskId, costCategoryObj) => {
        ...
        for (var opLineIndex = 0; opLineIndex < costdetailLineCount; opLineIndex++) {
            ...
            // var item =  recordData.getCurrentSublistText({
            //     sublistId: 'costdetail',
            //     fieldId: 'item'
            // })
