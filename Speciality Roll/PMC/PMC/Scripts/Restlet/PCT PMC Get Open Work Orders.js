            ...
            var manufacturingoperationtaskSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        // "AND",
                        // ["custrecord_pct_pmc_man_op_task.custrecord_pct_pmc_emp","anyof",dataObj.employeeId, "@NONE@"]
                    ],
                columns:
                    [
                        search.createColumn({
                        ...
                        search.createColumn({
                            name: "internalid",
                            join: "CUSTRECORD_PCT_pmc_MAN_OP_TASK",
                            summary: "MAX",
                            label: "Internal ID"
                        }),
                        search.createColumn({
                            name: "formulanumeric",
                            summary: "MAX",
                            formula: `CASE  WHEN {custrecord_pct_pmc_man_op_task.custrecord_pct_pmc_emp} = '${dataObj.employeeName}' THEN {custrecord_pct_pmc_man_op_task.internalid} ELSE 0 END`,
                ...
                for (let searchIndex = 0; searchIndex < searchResult.length; searchIndex++) {
                    ...
                    if (resData.pmcTransactionId) {
                        let pmcTransactionData = search.lookupFields({
                            ...
                            columns: ['custrecord_pct_pmc_emp', 'custrecord_pct_pmc_op_status', 'custrecord_pct_pmc_res_start_date', 'custrecord_pct_pmc_dwn_start_time', 'name', 'custrecord_pct_pmc_dwn_end_time', 'custrecord_pct_pmc_dwn_duration']
                        })
                        log.debug({
                            title: 'PCT-PMC',
                            details: `PMC Transaction Data = ${JSON.stringify(pmcTransactionData)}`
                        })
                        resData.pmcTransactionStatus = pmcTransactionData.custrecord_pct_pmc_op_status[0].text;
                        resData.pmcTransactionStatusId = pmcTransactionData.custrecord_pct_pmc_op_status[0].value;
                        resData.pmcTransactionResultStartTime = pmcTransactionData.custrecord_pct_pmc_res_start_date;
