...
define(['N/record', 'N/search'], function (record, search) {
    ...
    function _get(context) {
        try {
            ...
            let workInstructionDetails = getWorkInstructionData(workOrderId, operationSequence);//getPMCWorkInstructionData
            ...
            let itemAdditionalDetails = getItemAdditionalDetails(itemId)
            ...
            itemAdditionalDetails['columnsArr'] = generateTableHeader(itemAdditionalDetails.keys, tableHeaders.finishingArr)

            return { 'isSuccess': true, 'workOrderItemDetails': workOrderItemDetails, 'workOrderOperationTaskDetails': workOrderOperationTaskDetails, 'toolFamilyDetails': toolFamilyTaskDetails, 'pmcTransactionDetails': pmcTransactionDetails, 'serializedToolDetails': serializedToolTaskDetails, 'PMCworkInstructionDetails': PMCworkInstructionDetails, 'workInstructionDetails': workInstructionDetails, 'processInputDetails': processInputsDetails, 'itemAdditionalDetails': itemAdditionalDetails, 'isLastOperation': isLastOperation.isSuccess }
        }
        catch (error) {
    ...
    const getItemAdditionalDetails = (itemId) => {
        ...
        try {
            ...
            var itemSearchObj = search.create({
                ...
                filters:
                    [
                        ["internalid", "anyof", "148"]
                    ],
                columns:
                    [
                        search.createColumn({ name: "custitem_pct_ranger_lables_per_roll", label: "Labels Per Roll" }),
                        search.createColumn({ name: "custitem_pct_rangr_labels_on_rolls_she", label: "Labels on rolls / sheeted / fanfold" }),
                        search.createColumn({ name: "custitem_pct_ranger_core_od", label: "Core OD" }),
                        search.createColumn({ name: "custitem_pct_ranger_core_thickness", label: "Core Thickness" }),
                        search.createColumn({ name: "custitem_pct_ranger_roll_od", label: "Roll OD" }),
                        search.createColumn({ name: "custitem_pct_ranger_machine_count", label: "Machine count" }),
                        search.createColumn({ name: "custitem_pct_ranger_carton_size", label: "Carton Size" }),
                        search.createColumn({ name: "custitem_pct_ranger_rewind_direction", label: "Rewind direction" }),
                        search.createColumn({ name: "custitem_pct_ranger_tool_number", label: "Tool Number" }),
                        search.createColumn({ name: "custitem_pct_ranger_shape", label: "Shape" }),
                        search.createColumn({ name: "custitem_pct_ranger_anilox", label: "Anilox - line screen & BCM" })
                    ]
            });
            ...
            itemSearchObj.run().each(function (result) {
                let res = JSON.parse(JSON.stringify(keyObjects));
                res.coreod = result.getValue({ name: "custitem_pct_ranger_core_od" });
                res.labelsonrolls = result.getValue({ name: "custitem_pct_rangr_labels_on_rolls_she" });
                res.rollod = result.getValue({ name: "custitem_pct_ranger_roll_od" });
                res.machinecount = result.getValue({ name: "custitem_pct_ranger_machine_count" });
                res.labelsperroll = result.getValue({ name: "custitem_pct_ranger_lables_per_roll" });
                res.corethickness = result.getValue({ name: "custitem_pct_ranger_core_thickness" });
                res.cartonsize = result.getValue({ name: "custitem_pct_ranger_carton_size" });
                res.rewinddirection = result.getValue({ name: "custitem_pct_ranger_rewind_direction" });
                res.toolnumber = result.getValue({ name: "custitem_pct_ranger_tool_number" });
                res.shape = result.getValue({ name: "custitem_pct_ranger_shape" });
                res.aniloxLinescreen = result.getValue({ name: "custitem_pct_ranger_anilox" });


                itemDetailsArr.push(res)
                return true;
            });
            return { 'hasContent': true, 'data': itemDetailsArr, 'keys': keyObjects }
        }
        catch (error) {
            return { 'hasContent': false, 'data': itemDetailsArr, 'keys': keyObjects, 'errorMessage': error.message }
        }
    }
    ...
    const getTableHeaders = () => {
        return {
            ...
            // 'finishingArr': ['Labels Per Roll', 'Labels on rolls / sheeted / fanfold', 'Core OD', 'Core Thickness', 'Roll OD', 'Machine count', 'Carton Size', 'Rewind direction'],//, 'OPERATION NAME', 'WORK CENTER',
            // 'printingArr': ['Tool Number', 'Shape', 'Anilox - line screen & BCM'],//, 'OPERATION NAME', 'WORK CENTER',
            'itemAdditionalDetails': ['Labels Per Roll', 'Labels on rolls / sheeted / fanfold', 'Core OD', 'Core Thickness', 'Roll OD', 'Machine count', 'Carton Size', 'Rewind direction', 'Tool Number', 'Shape', 'Anilox - line screen & BCM'],
