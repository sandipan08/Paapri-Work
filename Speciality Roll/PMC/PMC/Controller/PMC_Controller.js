...
$(document).ready(async () => {
    ...
    $("#quality").on("click", async () => {
        ...
        if (
            ...
            for (var i = 0; i < drawingSpecificationTbody.length; i++) {
                ...
                console.log("noOfTesting HEADER", noOfTesting);
            ...
            console.log("maxHeader", maxHeader);
            ...
            for (
                ...
                console.log(
                    "drawingSpecificationTbody" + inspectionHeaderIndex,
                    drawingSpecificationTbody[inspectionHeaderIndex]
                );
                var noOfTesting =
                    drawingSpecificationTbody[inspectionHeaderIndex].noOfTesting.data;
                console.log("noOfTesting", noOfTesting);
    ...
    $(document).on("input", ".inspectionTextBox", (e) => {
        ...
        console.log(
            "qualityData.additionalData.sampleSize***",
            qualityData.additionalData.sampleSize
        );
        ...
        console.log("rowMinValue****", rowMinValue);
        console.log("lowerValue****", lowerValue);
...
const stopTimerClock = async () => {
    ...
    let completionObj = {
        'pmcTransactionId': selectedRowData.pmcTransactionId,
        'productionTime': (productionTime / 60000).toFixed(5),
    ...
    if (selectedRowData.pmcTransactionName.toLowerCase() === 'production') {
        ...
        let selectedRowWorkOrderId = selectedRowData.workOrderId;
        if (int)
            div = `<div>Quantity <input type="number" value="" class="swal2-input" id="productionQuantity" min='0' step='0.01'><span id="swal_uom" style="font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`;
        if (workOrderAdditionalData[selectedRowWorkOrderId].isLastOperation == true) {
            div += `<br/><div>Scrap<input type="number" value="" class="swal2-input" id="productionScrapQuantity" min='0' step='0.01'><span id="swal_uom" style="font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`;
        }
        clearInterval(int);
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                return { productionQuantity: productionQuantity, productionScrapQuantity: productionScrapQuantity }
            }
        }).then(async (result) => {
            if (result.isConfirmed) {
...
const getOpenWorkOrders = async () => {
    ...
    openWorkOrderDataTable.off('select').on('select', async () => {
        ...
        $('#itemfinishingSectionTable').DataTable({
            data: workOrderAdditionalData[selectedRowWorkOrderId].itemAdditionalDetails.data,
            columns: workOrderAdditionalData[selectedRowWorkOrderId].itemAdditionalDetails.columnsArr,
            processing: true,
            deferRender: true,
            destroy: true,
            "pageLength": 100,
            initComplete: function (settings, json) {
                $("#itemfinishingSectionTable").wrap("<div style='overflow:auto; width:100%;position:relative;'></div>");
            }
        });
        $('#assemblyItemTableContainer-span').html(workOrderAdditionalData[selectedRowWorkOrderId].itemAdditionalDetails.data.length)
