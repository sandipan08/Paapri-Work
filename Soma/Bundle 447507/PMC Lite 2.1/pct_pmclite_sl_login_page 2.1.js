    ...
    function (file, render, search, log, redirect, record, format) {
                ...
                if (isUserExisit.isSuccess) {
                    ...
                    redirect.toSuitelet({
                        ...
                        parameters: {
                            ...
                            'custparam_workCenter': isUserExisit.workCenterId,
                            'custparam_workCenterName': isUserExisit.workCenterName,
        ...
        function IsUserExist(userPin) {
            ...
            var employeeSearchObj = search.create({
                type: "employee",
                filters: [["custentity_pct_soma_work_center", "noneof", "@NONE@"],
                    "AND", ["custentity_pct_cit_badge_id", "is", userPin]],
                columns: [search.createColumn({ name: "entityid", sort: search.Sort.ASC, label: "Name" }),
                search.createColumn({ name: "internalid", label: "Internal ID" }),
                search.createColumn({ name: "custentity_pct_soma_work_center", label: "PMC Work Center" })
                ]
            });
            ...
            if (searchResultCount > 0) {
                employeeSearchObj.run().each(function (result) {
                    ...
                    user.workCenterId = result.getValue({ name: 'custentity_pct_soma_work_center' });
                    user.workCenterName = result.getText({ name: 'custentity_pct_soma_work_center' });
                    user.isSuccess = true;
                    return true;
                });
            } else {
