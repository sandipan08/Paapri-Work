...
define(['N/file', 'N/render', 'N/search', 'N/log', 'N/redirect', 'N/url', 'N/record', 'N/format'],
    function (file, render, search, log, redirect, url, record, format) {
        ...
        function onRequest(context) {
            ...
            if (_request.method == 'GET') {
                ...
                log.debug("PCT-Get", "Parameters : " + JSON.stringify(_request.parameters))
                ...
                log.debug("PCT_GET", "WOrk center id : " + workCenter + ", Name : " + workCenterName)
                ...
                log.debug("PCT", "ticketNumber  : " + ticketNumber)
                ...
                log.debug("PCT", "getOperationStatus  : " + getOperationStatus)
                ...
                if (backFromBreak == "BREAK_COMPLETE") {
                    ...
                    if (mode == "VIEW_MODE" || mode == "START_MODE") {
                        ...
                        //  var operationInternalId = getOperationId(workCenter, ticketNumber).operationId;
                        //  var operationNumber = getOperationId(workCenter, ticketNumber).operationSequence;
                        log.debug("PCT Get View Mode", "Call GetManufacturingOperationDetails function")
                        var mfgOperationSearchObj = GetManufacturingOperationDetails(workCenter, ticketNumber);
                        var operationInternalId = mfgOperationSearchObj.internalid;
                        var operationNumber = mfgOperationSearchObj.sequence;
                        log.debug("PCT Manufacture Log", "Operation Internal Id : " + operationInternalId + ", Number : " + operationNumber)
                        if (mode == "VIEW_MODE") {
                            // Case-1
                            // Go to the Home Page
                            // NOte: Mode = "VIEW_MORE"
                            PageRendererByMode(userId, userName, todayQuantity, loginRecordId, 0, logoutPageUrl,
                                ...
                                "VIEW_MODE", mfgOperationSearchObj, workCenter, workCenterName);
                        ...
                        if (mode == "START_MODE") {
                            ...
                            PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl,
                                ...
                                mfgOperationSearchObj, workCenter, workCenterName);
                        }
                        ...
                        PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, breakPageUrl, homePageUrl,
                            woCompletionUrl, qrCode, 'HOME_PAGE', getOperationStatus, workCenter, workCenterName);
                    }
                ...
                log.debug("PCT_ELSE", "Ticket Number : " + ticketNumber + ", workCenter : " + workCenter + ", Mode : " + mode + ", Break_end : " + Break_end);
                ...
                if (Break_end == 'Break_end') {
                    ...
                    log.debug("PCT Post Break End", "Call GetManufacturingOperationDetails function")
                    var mfgOperationSearchObj = GetManufacturingOperationDetails(workCenter, ticketNumber);
                    var operationInternalId = mfgOperationSearchObj.internalid;
                    var operationNumber = mfgOperationSearchObj.sequence;
                    ...
                    PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, breakPageUrl,
                        homePageUrl, woCompletionUrl, ticketNumber, "BREAK_END_MODE", mfgOperationSearchObj, workCenter, workCenterName);//"BREAK_END_MODE",
                }
                else {
                    // Getting Manufacturing Operation Task Details
                    log.debug("PCT Post Break end else", "Call GetManufacturingOperationDetails function")
                    var mfgOperationSearchObj = GetManufacturingOperationDetails(workCenter, ticketNumber);
                    var operationInternalId = mfgOperationSearchObj.internalid;
                    var operationNumber = mfgOperationSearchObj.sequence;
                    ...
                    if (mfgOperationSearchObj.isSuccess) {
                        ...
                        if (status == "COMPLETE") {
                            log.debug("PCT COmplete 1")
                            // This block is execute when the operation task status is "COMPLETE"
                            // Go to the Home Page
                            // Note: Status = "COMPLETE"
                            log.debug("PCT-COMPLETE", 'workCenter : ' + workCenter)
                            PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, breakPageUrl,
                                homePageUrl, woCompletionUrl, ticketNumber, status, getOperationStatus, workCenter, workCenterName);

                        } else {
                            // This block is execute only when the operation task status = NOTSTARTED || INPROGRESS
                            if (mode == "VIEW_MODE") {
                                log.debug("PCT View 1")
                                // Go to the Home Page
                                // NOte: Mode = "VIEW_MORE"
                                log.debug({
                                    title: 'View Mode Log',
                                    details: operationInternalId + ", Work Center Name : " + workCenterName
                                })
                                PageRendererByMode(userId, userName, todayQuantity, loginRecordId, 0, logoutPageUrl, breakPageUrl,
                                    homePageUrl, woCompletionUrl, ticketNumber, "VIEW_MODE", mfgOperationSearchObj, workCenter, workCenterName);

                            } else { // START_MODE
                                log.debug("PCT Else 1")
                                // Creating PMC Transaction table for bundle start
                                var startRecordId = CreateTransactionRecord(userId, "Running", mfgOperationSearchObj.internalid, mfgOperationSearchObj.sequence, mfgOperationSearchObj);//"Bundle Start"//In Process
                                // Go to the Home Page
                                // NOte: Mode = "START_MODE"
                                PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl,
                                    breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber,
                                    "START_MODE", mfgOperationSearchObj, workCenter, workCenterName);
                            }
                        }
                        ...
                        // NOte: Mode = "START_MODE"
                        PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, breakPageUrl, homePageUrl,
                            woCompletionUrl, ticketNumber, _status, getOperationStatus, workCenter, workCenterName);
                    }
         ...
         * @param {string} status 
         */
        function PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, status, getOperationStatus, workCenter, workCenterName) {
            ...
            var dataSource = {
                ...
                tickerNumberStatus: checkTicketNumber(ticketNumber)
            };
         ...
         */
        function PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, mode, dataObj, workCenter, workCenterName) {
            log.debug({
                title: 'View Mode Log Page Render',
                details: {
                    mode: mode,
                    internalId: dataObj.internalid,
                    workCenter: workCenter,
                    workCenterName: workCenterName,
            ...
            var operationId = dataObj.internalid;
            ...
            var dataSource = {
                ...
                sequenceId: dataObj.sequence,
                workorder: dataObj.workorder,
                workOrderTranid: dataObj.workOrderTranid,
                item: dataObj.item,
     ...
     */
         ...
         */
        function GetManufacturingOperationDetails(workCenter, workOrderId) {
            log.debug("PCT Search", "Work order incoming Id : " + workOrderId + ", work center : " + workCenter)

            let ticketNumber = workOrderId;
            workOrderId = parseInt(workOrderId.split('O')[1].split('-')[0]);
            log.debug("PCT Search", "Work order no after : " + workOrderId)
            ...
            var mfgOpTaskSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        "AND",
                        ["workorder.number", "equalto", workOrderId]
            ...
            if (searchResultCount > 0) {
                mfgOpTaskSearchObj.run().each(function (result) {
                    ...
                    searchResults.workOrderTranid = result.getValue({
                        name: "tranid",
                        join: "workOrder"
                    })
            ...
            log.debug({
                title: 'GetManufacturingOperationDetails function searchResults ',
        ...
        function CreateTransactionRecord(userId, actionType, operationInternalId, operationNumber, mfgOperationSearchObj) {
            log.debug("PCT_CREATE PMC Transaction ", "Internal Id : " + operationInternalId + 'operationNumber : ' + operationNumber)
            var now = new Date();
        ...
        */
        ...
        const checkTicketNumber = (ticketNumber) => {
            let searchResultCount = 0;
            log.debug("PCT", "Ticket Number : " + ticketNumber)
            var workordercompletionSearchObj = search.create({
                ...
                filters:
                    [
                        ["type", "anyof", "WOCompl"],
                        "AND",
                        ["custbody_pct_soma_traveler_tckt_no", "is", ticketNumber],
                        "AND",
                        ["custbody_pct_soma_traveler_tckt_no", "isnotempty", ""],
                        "AND",
                        ["mainline", "is", "T"]
                    ],
                columns:
            ...
            searchResultCount = workordercompletionSearchObj.runPaged().count;

            log.debug("PCT-Soma", "Ticket Number Present in Work Order Completion Count :" + searchResultCount);
        ...
        */
        ...
        const getOperationId = (workCenter, workOrderId) => {
            log.debug("PCT", "Work Center : " + workCenter + ", Work Order Id : " + workOrderId)
            ...
            let operationTaskObj = {};
            if (searchResultCount > 0) {
                manufacturingoperationtaskSearchObj.run().each(function (result) {
                    operationTaskObj['isSuccess'] = true;
                    operationTaskObj['operationId'] = result.id;
                    operationTaskObj['operationName'] = result.getValue('name');
                    operationTaskObj['operationSequence'] = result.getValue('sequence');
                    return true;
                });
            }
            else {
                operationTaskObj['isSuccess'] = false;
            }
            return operationTaskObj;
        }
