/**
 *              //////////     PMC QMS 2.1 | Filler Testing Form Page    //////////
 * 
 *@author       Sandipan Sau
 *@NApiVersion  2.1
        ...
        function onRequest(context)
        {
            ...
            if (request.method == 'GET')
            {
                ...
                else if (documentStatus == "Existing")
                {
                    ...
                    log.debug({
                        title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName +
                            ", Vehicle No :" + vehicleNo + ", Brightness : " + brightness + ", Oil Absorption : " + oilAbsorption + ", Water Absorption : " + waterAbsorption +
                            ", PH Value : " + phValue + ", Sp Gravity :" + spGravity + ", HCL Reaction : " + hclReaction + ", Mesh Analysis : " + meshAnalysis + ", QC Approved : " + qcChecked + ", Report : " + report
                    });

                }
                // Load Login HTML Template
                var templateFile = file.load({ id: '../../HTML Files/Forms/pct_qms_filler_testing_report_form.html' });
            ...
            else
            {
                ...
                //  var fileName = request.parameters.customFile;
                ...
                log.debug({
                    title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName +
                        ", Vehicle No :" + vehicleNo + ", Brightness : " + brightness + ", Oil Absorption : " + oilAbsorption + ", Water Absorption : " + waterAbsorption +
                        ", PH Value : " + phValue + ", Sp Gravity :" + spGravity + ", HCL Reaction : " + hclReaction + ", Mesh Analysis : " + meshAnalysis + ", Report : " + report + ", QC Checked : " + qcChecked
                });
                // -------------------------- Save Value in Custom Record ------------------------------
                var fillerFormObj = record.create({ type: "customrecord_pct_pp_qms_record", isDynamic: true });
                fillerFormObj.setText({ fieldId: 'customform', text: "Filler Testing Report Form" });
                if (documentType == "itemreceipt")
                {
                    fillerFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Item Receipt" });
                }
                else if (documentType == "assemblybuild")
                {
                    fillerFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Assembly Build" });
                }
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_document_number', value: documentNumber });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_date', value: entryDate });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_material_name_pdct_nme', value: productName });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_party_name', value: partyName });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_vehicles_no', value: vehicleNo });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_brightness', value: brightness });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_oil_absorption', value: oilAbsorption });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_water_absorption', value: waterAbsorption });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_ph_value', value: phValue });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_sp_gravity', value: spGravity });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_hcl_reaction', value: hclReaction });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_mesh_analysis', value: meshAnalysis });
                fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_report', value: report });
                if (qcChecked === "on")
                {
                    fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: true });
                }
                else
                {
                    fillerFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: false });
                }
                var qmsRecordId = fillerFormObj.save();
