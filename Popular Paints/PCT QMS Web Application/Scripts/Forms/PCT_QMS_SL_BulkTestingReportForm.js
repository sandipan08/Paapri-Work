/**
 *              //////////     PMC QMS 2.1 | Bulk Testing Form Page    //////////
 * 
 *@author       Sandipan Sau
 *@NApiVersion  2.1
    ...
    function (file, render, search, log, redirect, record, format, email, runtime, url)
    {
        ...
        function onRequest(context)
        {
            ...
            if (request.method == 'GET')
            {
                ...
                log.debug({ title: 'PCT-QMS', details: "User Name : " + custparam_userName + ", Document Type : " + documentType + ", Document Number : " + documentNumber + ", Document Status : " + documentStatus + ", Product Name : " + productName + ", Form : " + qmsForm + ", Serial Number : " + serialNumber + ", Party Name : " + partyName });
                ...
                else if (documentStatus == "Existing")
                {
                    var customrecord_pct_pp_qms_recordSearchObj = search.create({
                        ...
                        filters:
                            [
                                ...
                                "AND",
                                ["custrecord_pct_pp_material_name_pdct_nme", "is", productName],
                        ...
                        columns:
                            [
                                ...
                                search.createColumn({ name: "custrecord_pct_pp_manual_measured_dip", label: "Manual Measured Dip" }),
                                search.createColumn({ name: "custrecord_pct_pp_weight_per_ltr", label: "Weight Per Ltr" }),
                                search.createColumn({ name: "custrecord_pct_pp_drying_time", label: "Drying Time" }),
                                search.createColumn({ name: "custrecord_pct_pp_colour_maching", label: "Colour Maching" }),
                                search.createColumn({ name: "custrecord_pct_pp_viscosity_at_30_degree", label: "Viscosity at 30 degree" }),
                    ...
                    for (var qms_index = 0; qms_index < qmsRecordCount; qms_index++)
                    {
                        ...
                        var manualMeasuredDip = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_manual_measured_dip');
                        var weightPerLtr = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_weight_per_ltr');
                        var dryingTime = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_drying_time');
                        var colourMaching = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_colour_maching');
                        var viscosityAt30Degree = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_viscosity_at_30_degree');
                    ...
                    var dataSource = {
                        ...
                        partyName: partyName,
                        vehicleNo: vehicleNo,
                        manualMeasuredDip: manualMeasuredDip,
                        weightPerLtr: weightPerLtr,
                        dryingTime: dryingTime,
                        colourMaching: colourMaching,
                        viscosityAt30Degree: viscosityAt30Degree,
                        report: report,
                        qcChecked: qcChecked,
                        logoutPageUrl: logoutPageUrl

                    };
                    log.debug({ title: 'PCT-QMS', details: "User Name : " + custparam_userName + ", Document Type : " + documentType + ", Document Number : " + documentNumber + ", Form : " + qmsForm });
                    log.debug({
                        title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName + ", Vehicle No :" + vehicleNo + ", Manual Measured Dip : " + manualMeasuredDip +
                            ", Weight Per Ltr : " + weightPerLtr + ", Dying Time :  " + dryingTime + ", Color Machine : " + colourMaching + ", Viscosity At30 : " + viscosityAt30Degree + ", QC Approved : " + qcChecked + ", Report : " + report
                    });

                }
                // Load Login HTML Template
                var templateFile = file.load({ id: '../../HTML Files/Forms/pct_qms_bulk_testing_report_form.html' });
            ...
            else
            {
                ...
                var manualMeasuredDip = request.parameters.manualMeasuredDip;
                var weightPerLtr = request.parameters.weightPerLtr;
                var dryingTime = request.parameters.dryingTime;
                var colourMaching = request.parameters.colourMaching;
                var viscosityAt30Degree = request.parameters.viscosityAt30Degree;
                ...
                var bulkFormObj = record.create({ type: "customrecord_pct_pp_qms_record", isDynamic: true });
                bulkFormObj.setText({ fieldId: 'customform', text: "Solvent Testing Form" });
                if (documentType == "itemreceipt")
                {
                    bulkFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Item Receipt" });
                }
                else if (documentType == "assemblybuild")
                {
                    bulkFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Assembly Build" });
                }
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_document_number', value: documentNumber });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_date', value: entryDate });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_material_name_pdct_nme', value: productName });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_party_name', value: partyName });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_vehicles_no', value: vehicleNo });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_manual_measured_dip', value: manualMeasuredDip });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_weight_per_ltr', value: weightPerLtr });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_drying_time', value: dryingTime });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_colour_maching', value: colourMaching });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_viscosity_at_30_degree', value: viscosityAt30Degree });
                bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_report', value: report });
                if (qcChecked === "on")
                {
                    bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: true });
                }
                else { bulkFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: false }); }
                var qmsRecordId = bulkFormObj.save();
