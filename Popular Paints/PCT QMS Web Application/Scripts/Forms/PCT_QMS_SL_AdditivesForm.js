/**
 *              //////////     PMC QMS 2.1 | Additives Testing Report Form    //////////
 * 
 *@author       Sandipan Sau
 *@NApiVersion  2.1
        ...
        function onRequest(context)
        {
            ...
            if (request.method == 'GET')
            {
                ...
                else if (documentStatus == "Existing")
                {
                    ...
                    log.debug({
                        title: "PCT_QMS",
                        details: "Additive Form Data : ( Batch No : " + batchNo + ", Party Name : " + partyName + ", Vehicle No : " + vehicleNo + ", Sp Gravity : " + spGravity + ", Viscosity : " + viscosity + ", Temperature : " + temperature + ", Ph Value : " + phValue + ", Solid : " + solid + ", Color : " + color + ", Report : " + report + " )"
                    })
                    // Assemble Data Source for Home Page
                    var dataSource = {
                        faviconUrl: faviconUrl,
                    ...
                    // log.debug({
                    //     title: 'PCT-QMS-Getmethod', details: "Product Name : " + productName + ", Party Name : " + partyName + ", Vehicle No :" + vehicleNo + ", Color : " + color + ", PH Value : " + phValue + ", Sp Gravity :" + spGravity + ",  Temperature : " + temperature + ", Report : " + report
                    // });
                }
                // Load Login HTML Template
                var templateFile = file.load({ id: '../../HTML Files/Forms/pct_qms_additives_testing_report_form.html' });
            ...
            else
            {
                ...
                var solid = request.parameters.solidPercentage;
                ...
                log.debug({
                    title: 'PCT-QMS', details: "Product Name : " + productName + ", Party Name : " + partyName + ", Vehicle No :" + vehicleNo + ", Color : " + color + ", PH Value : " + phValue + ", Sp Gravity :" + spGravity + ",  Temperature : " + temperature + ", Report : " + report + ", QC Checked : " + qcChecked
                });
                // -------------------------- Save Value in Custom Record ------------------------------
                var additivesFormObj = record.create({ type: "customrecord_pct_pp_qms_record", isDynamic: true });
                additivesFormObj.setText({ fieldId: 'customform', text: "Additives Testing Report Form" });
                if (documentType == "itemreceipt")
                {
                    additivesFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Item Receipt" });
                }
                else if (documentType == "assemblybuild")
                {
                    additivesFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Assembly Build" });
                }
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_document_number', value: documentNumber });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_material_name_pdct_nme', value: productName });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });

                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_viscosity_solution', value: viscosity });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_vehicles_no', value: vehicleNo });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_party_name', value: partyName });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_sp_gravity', value: spGravity });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_temperature', value: temperature });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_ph_value', value: phValue });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_colour', value: color });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_solid', value: solid });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_report', value: report });
                additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });
                if (qcChecked === "on")
                {
                    additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: true });
                }
                else { additivesFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: false }); }
                var qmsRecordId = additivesFormObj.save();
