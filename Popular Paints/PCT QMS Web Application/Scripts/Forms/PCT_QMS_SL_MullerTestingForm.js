/**
 *              //////////     PMC QMS 2.1 | Muller Grinding Testing Form Page    //////////
 * 
 *@author       Sandipan Sau
 *@NApiVersion  2.1
    ...
    function (file, render, search, log, redirect, record, format, email, runtime, url)
    {
        ...
        function onRequest(context)
        {
            ...
            if (request.method == 'GET')
            {
                ...
                else if (documentStatus == "Existing")
                {
                    var customrecord_pct_pp_qms_recordSearchObj = search.create({
                        ...
                        columns:
                            [
                                ...
                                search.createColumn({ name: "custrecord_pct_pp_result", label: "Result" }),
                    ...
                    for (var qms_index = 0; qms_index < qmsRecordCount; qms_index++)
                    {
                        ...
                        var result = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_result');
                    ...
                    log.debug({
                        title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName + ", Result : " + result +
                            ", Vehicle No :" + vehicleNo + ", QC Approved : " + qcChecked + ", Report : " + report
                    });

                }
                // Load Login HTML Template
                var templateFile = file.load({ id: '../../HTML Files/Forms/pct_qms_muller_grinding_testing_report_form.html' });
            ...
            else
            {
                ...
                var result = request.parameters.result;
                ...
                log.debug({
                    title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName +
                        ", Vehicle No :" + vehicleNo + ", Result : " + result + ", QC Approved : " + qcChecked + ", Report : " + report
                });
                // -------------------------- Save Value in Custom Record ------------------------------
                var mullerFormObj = record.create({ type: "customrecord_pct_pp_qms_record", isDynamic: true });
                mullerFormObj.setText({ fieldId: 'customform', text: "Solvent Testing Form" });
                if (documentType == "itemreceipt")
                {
                    mullerFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Item Receipt" });
                }
                else if (documentType == "assemblybuild")
                {
                    mullerFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Assembly Build" });
                }
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_document_number', value: documentNumber });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_date', value: entryDate });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_material_name_pdct_nme', value: productName });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_party_name', value: partyName });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_vehicles_no', value: vehicleNo });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_result', value: result });
                mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_report', value: report });
                if (qcChecked === "on")
                {
                    mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: true });
                }
                else { mullerFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: false }); }
                var qmsRecordId = mullerFormObj.save();
