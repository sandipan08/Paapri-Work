...
*@description  This WorkflowActionScript is used to create Sales Order,Invoice, Item Fulfillment, Refund from Custom Record.
*/
...
define(['N/log', 'N/record', 'N/runtime', 'N/file', 'N/format', 'N/search', 'N/email', 'N/runtime'], function (log, record, runtime, file, format, search, email, runtime)
{
    ...
    var generateFulfillment = generateInvoice = true;
    var subsidiary = 0;
    var discountItem = 0;
    var shippingItemId = 0;
    var salesOrderLocation = "";

    function onAction(scriptContext)
    {
        log.debug({ title: 'PCT-Shopify-Integration', details: "In WorkFlow OnAction Function" });
        // -------------------------------------------------------- Get All Preferences -----------------------------------------------------------------
        var customrecord_pct_shopify_preference_formSearchObj = search.create({
            type: "customrecord_pct_shopify_preference_form",
            filters:
                [
                ],
            columns:
                [
                    search.createColumn({ name: "custrecord_pct_generate_fulfillment", label: "Generate Fulfillment" }),
                    search.createColumn({ name: "custrecordshopify_preference_gene_bill", label: "Generate Invocie" }),
                    search.createColumn({ name: "custrecord_pct_shopify_preference_subs", label: "Subsidiary" }),
                    search.createColumn({ name: "custrecord_shopify_discount_itemid", label: "Discount Item Id" }),
                    search.createColumn({ name: "custrecord_shopify_shipping_itemid", label: "Shipping Method" }),
                    search.createColumn({ name: "custrecord_shopify_preference_solocation", label: "Sales order Location" }),
                    search.createColumn({
                        name: "custrecord_pct_market_place_name",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Market Place Name"
                    }),
                    search.createColumn({
                        name: "custrecord_pct_country_code",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Country Code"
                    }),
                    search.createColumn({
                    ...
                    search.createColumn({
                        name: "custrecord_pct_exclude_tax",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Exclude Tax ( Item + Shipping )"
                    }),
                    search.createColumn({
                        name: "custrecord_pct_fulfillment_location",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Fulfillment Location"
                    }),
                    search.createColumn({
                        name: "custrecord_pct_shopify_pref_subsidiary",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Subsidiary"
                    }),
                    search.createColumn({
                        name: "custrecord_pct_shopify_customer_name",
                        join: "CUSTRECORD_PCT_LINK_TO_SHOPIFY_PREFERENC",
                        label: "Customer Name"
                    })
        ...
        var shopifyPreferencesCount = customrecord_pct_shopify_preference_formSearchObj.runPaged().count;
        log.debug("PCT-Shopify-Integration", "Shopify Preferences Count : " + shopifyPreferencesCount);
        var shopifyPreferencesResult = customrecord_pct_shopify_preference_formSearchObj.run().getRange({ start: 0, end: shopifyPreferencesCount });
        if (shopifyPreferencesCount > 0)
        {
            for (var preferencesIndex = 0; preferencesIndex < shopifyPreferencesCount; preferencesIndex++)
            {
                generateFulfillment = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecord_pct_generate_fulfillment" });
                generateInvoice = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecordshopify_preference_gene_bill" });
                subsidiary = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecord_pct_shopify_preference_subs" });
                discountItem = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecord_shopify_discount_itemid" });
                shippingItemId = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecord_shopify_shipping_itemid" });
                salesOrderLocation = shopifyPreferencesResult[preferencesIndex].getValue({ name: "custrecord_shopify_preference_solocation" });


                log.debug({
                    title: "PCT-Shopify-Integration",
                    details: "Shopify Preference Order Details : [ Generate Fulfillment : " + generateFulfillment + ", Generate Invoice : " + generateInvoice + ", Subsidiary : " + subsidiary + ", Discount Item Id : " + discountItem + ", Shipping Method Id : " + shippingItemId + ", Sales Order Location : " + salesOrderLocation + " ]"
                })
            }
        }
        ...
        try
        {
            ...
            var customerId = createCustomer(shopify_customerName);
            ...
            salesOrderobj.setText({ fieldId: 'location', text: salesOrderLocation });
            ...
            subrec.setValue({ fieldId: 'addr2', value: shopify_shippingAddresss2 });
            ...
            else
            {
                ...
                if (Math.abs(orderDiff).toFixed(2) != 0.00)
                {
                    ...
                    for (var index = 0; index < so_item_count; index++)
                    {
                        ...
                        if (itemType != "Group" && itemType != "EndGroup")
                        {
                            ...
                            if (salesOrderTotal > shopify_totalPrice)
                            { var newAmount = ((parseFloat(itemamt) - parseFloat(orderDiff))).toFixed(2); }
                            else
                            { var newAmount = ((parseFloat(itemamt) + parseFloat(orderDiff))).toFixed(2); }
                ...
                if (generateFulfillment)
                {
                    ...
                    itemFullfillment(salesorder_id, total_item_count, total_fulfillment_item_count, shopify_item_count, shopify_fulfillment_count, refundObjLen);
                }

                //--------------------------------------------- Billing ----------------------------------------------
                if (generateInvoice) //!invoiceInternalNo.length
                {
        ...
        {
            log.debug({
                title: "PCT-Shopify-Integration Create Customer Function", details: "Customer NAme :" + cName
            })
            ...
            log.debug("PCT-Shopify-Integration Create Customer Function", "Customer Count : " + customer_count);
            var customer_searchresult = customerSearchObj.run().getRange({ start: 0, end: customer_count });
            if (customer_count > 0)
            {
                log.debug({ title: "PCT-Shopify-Integration Create Customer Function", details: "Customer " + cName + " is Already Present " })
            ...
            {
                log.debug({ title: "PCT-Shopify-Integration Create Customer Function", details: "Creating a Customer" })
                ...
                log.debug({ title: "PCT-Shopify-Integration Create Customer Function", details: "New Customer Created & Customer Id " + customer_internal_id })
                return customer_internal_id;
            }
        }
        ...
        //-------------------------------------------------------------- Cancel Sales Order Functuion Start -----------------------------------------------
        ...
        //-------------------------------------------------------------- Cancel Sales Order Functuion End -----------------------------------------------
