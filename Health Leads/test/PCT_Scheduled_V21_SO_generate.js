...
Script Name:        PCT_Scheduled_V21_SO_generate
...
execute()						    	Get those hl web order id which sales order not genereated                          		   		    Sandipan Sau
                                        and Create json request (HL_record_array) and push json into file cabinet 
                                        and delete those record           
...
define(['N/log', 'N/record', 'N/runtime', 'N/file', 'N/format', 'N/search'], function (log, record, runtime, file, format, search)
{
    ...
    function execute(context)
    {
        ...
        var customrecord_pct_hl_web_orderSearchObj = search.create({
            ...
            filters:
                [
                    ...
                    ["created", "on", "06/04/2021 11:59 pm"] //for testing purpose
                ],
            columns:
                [
        ...
        log.debug("HL ID ResultCount", HL_id_ResultCount);
        //  customrecord_pct_hl_web_orderSearchObj.run().each(function(result){
        ...
        for (var getid_index = 0; getid_index < HL_id_ResultCount; getid_index++)
        {
            ...
            salesOrderobj.setValue({ fieldId: 'shipmethod', value: 408 }); // hard core value putting
        ...
        // log.debug({ title: "PCT-HL- HL Web Order Id Array ", details: "Record Array " + id_array })
