...
let restletsScriptObj = {
    ...
    'customscript_pct_sc_get_item_bin_numbers': {
        'scriptName': 'PCT PMC Get Item Numbers',
        'scriptId': 'customscript_pct_sc_get_item_bin_numbers',
        'scriptInternalId': ''
    }
}
...
$(document).ready(async () => {
    ...
    $('#logIn').click(async (e) => {
        ...
        if (employeeDetailsObj.responseSuccess && employeeDetailsObj.responseData.isSuccess) {
            ...
            // }
            if (loggedInUserDetails.blockedPMCIssue) {

                $('#workOrderIssue').attr('hidden', true)
...
const stopTimerClock = async () => {
    ...
    //     const binResponse = await fetchPostRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_sc_get_item_bin_numbers.scriptInternalId}&deploy=1`, { 'itemId': selectedRowData.itemId, 'workOrderId': selectedRowData.workOrderId, 'pmcTransactionId': selectedRowData.operationSequence })
    //     console.log(binResponse)
    //     let binDropdown = ''
    //     if (binResponse.responseSuccess && binResponse.responseData.isSuccess) {
    //         binDropdown = `<span 
    //           id="itemBinNumber">
    //            ${generateSelectField({ 'fieldId': 'itemBinNumber', 'fieldName': 'Bin Number', 'options': binResponse.responseData.data })}</span>
    //           `
    //     }
    ...
    //             console.log(Swal.getPopup().querySelector('#itemBinNumber'))
    //             const binNumber = Swal.getPopup().querySelector('#itemBinNumber').value
    //             console.log("BIn Nuber : " + binNumber)
    ...
    //             }
    //             else if (binNumber == '') {
    //                 Swal.showValidationMessage(`Please select a Bin Number`)
    //             }
    //             return { productionQuantity: productionQuantity, binNumber: binNumber }
    //         }
    //     }).then(async (result) => {
    //         if (result.isConfirmed) {
    //             let qtyEntered = result.value.productionQuantity
    //             let binNumber = result.value.binNumber
    //             console.log("BIn Nuber : " + binNumber)
    //             if (!selectedRowData.predecessorId) {
    //                 completionObj.productionQuantity = qtyEntered;
    //                 completionObj.binNumber = 33//binNumber;
    //             }
    //             else {
...
const getOpenWorkOrders = async () => {
    ...
    openWorkOrderDataTable = $('#openWorkOrderTable').DataTable({
        ...
        "columnDefs": [
            {
                "targets": [1, 3, 4, 5, 6, 10, 15, 16, 18, 20, 21, 22, 24, 25, 26, 27],// 17, 19
                "visible": false,
                className: 'dt-body-left'
    ...
    openWorkOrderDataTable.off('select').on('select', async () => {
        ...
        let informationObj = {
            ...
            'units': {
                'fieldName': 'Units',
                'fieldId': 'units',
                'fieldValue': selectedRowData.units
            },
            'status': {
                'fieldName': 'Status',
                'fieldId': 'status',
