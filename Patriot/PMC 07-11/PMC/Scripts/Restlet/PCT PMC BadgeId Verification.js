    ...
    const verifyEmployeeWithBadgeId = (badgeId) => {
        var getEmployeeSearch = search.create({
            ...
            columns:
                [
                    ...
                    search.createColumn({ name: "custentity_pct_pat3_blocked_pmc_issued", label: "Blocked PMC Issued" })
                ]
        });
        ...
        if (employeeCount > 0) {
            ...
            getEmployeeSearch.run().each(function (result) {
                ...
                employeeData['blockedPMCIssue'] = result.getValue({ name: "custentity_pct_pat3_blocked_pmc_issued", label: "Blocked PMC Issued" });
