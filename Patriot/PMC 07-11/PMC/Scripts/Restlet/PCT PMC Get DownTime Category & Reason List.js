...
define(['N/search'], function (search) {
    ...
    function _get(context) {
        log.debug("PCT-PMC", "In PCT PMC Get DownTime Category List " + context.categoryId);
        ...
        if (context.categoryId == 0) {
            ...
            if (fetchAllDownTimeCategory().isSuccess && fetchAllDownTimeReason().isSuccess) {
                ...
                return { 'isSuccess': false, 'errorMessage': 'DownTime Reason List Not Found' }
            }
        }
        else if (context.categoryId != 0) {

            if (fetchCategoryWiseDownTimeReason(context.categoryId).isSuccess) {
                return { 'isSuccess': true, 'reason': { 'fieldId': 'reasonType', 'fieldName': 'Downtime Reason', 'options': fetchCategoryWiseDownTimeReason(context.categoryId).data } }
            }
    ...
    const fetchCategoryWiseDownTimeReason = (categoryId) => {
        ...
        var customrecord_pct_pmc_dwn_reasonSearchObj = search.create({
            ...
            filters:
                [
                    ["custrecord_pct_pmc_dwn_category", "anyof", categoryId],
