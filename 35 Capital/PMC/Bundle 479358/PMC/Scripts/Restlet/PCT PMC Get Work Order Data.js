...
define(['N/record', 'N/search'], function (record, search) {
        ...
        try {
            ...
            if (itemLineCount > 0) {
                for (let lineIndex = 0; lineIndex < itemLineCount; lineIndex++) {
                    ...
                    res.issueQuantity = `<input type="number" class="form-control issueQuantity" value="${workOrderRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'quantitycommitted',
                        line: lineIndex
                    })}">`
    ...
    const getWorkInstructionData = (workOrderId) => {
        ...
        try {
            var customrecord_pct_pmc_work_instructionSearchObj = search.create({
                type: "customrecord_pct_sc_work_instructions_wo",
                filters:
                    [
                        ["custrecord_pct_str_link_field_wo", "anyof", workOrderId]
                    ],
                columns:
                    [
                        search.createColumn({ name: "custrecord_pct_str_operation_sequence_wo", label: "OPERATION SEQUENCE" }),
                        ...
                        search.createColumn({ name: "custrecord_pct_sc_workins_wo", label: "WORK INSTRUCTIONS" }),
                        search.createColumn({ name: "custrecord_pct_wo_ins_url", label: "WORK INSTRUCTIONS URL" })
                    ]
            });
            ...
            if (searchResultCount > 0) {
                customrecord_pct_pmc_work_instructionSearchObj.run().each(function (result) {
                    ...
                    let res = JSON.parse(JSON.stringify(keyObjects));
                    res.operationSequence = result.getValue({ name: "custrecord_pct_str_operation_sequence_wo" })
                    ...
                    res.workInstruction = result.getValue({ name: "custrecord_pct_sc_workins_wo" })
                    var url = result.getValue({ name: "custrecord_pct_wo_ins_url" })
    ...
    const getTableHeaders = () => {
        return {
            'itemsTableTitleArr': ['Item', 'Item Id', 'Total Qty on Hold', 'Total Qty Issued to this WO', 'Remaining Qty', 'Back Ordered', 'Component Yield', 'Qty with Zero Waste', 'Unit of Measure', 'Line No','Qty Used'],
            ...
            'workInstructionTitleArr': ['OPERATION SEQUENCE', 'OPERATION NAME', 'WORK CENTER', 'WORK INSTRUCTION', 'URL'],
