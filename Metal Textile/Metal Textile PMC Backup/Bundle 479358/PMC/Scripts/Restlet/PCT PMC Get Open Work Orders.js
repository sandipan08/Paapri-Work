...
define(['N/search', 'N/file'], function (search, file) {
    ...
    function _get(context) {
        ...
        const titleArr = ['Work Order', 'Work Order Id', 'Item', 'Item Id', 'Location', 'Location Id', 'Operation Id', 'Operation Sequence', 'Work Center', 'Work Center Id', 'Start Date', 'End Date', 'Input Quantity', 'Completed Quantity', 'Status', 'Customer', 'Created From', 'Customer PO', 'Run Rate', 'Predecessor', 'Predecessor Id', 'Predecessor Completed Quantity', 'Pmc Transaction Id', 'Result Start Time', 'Downtime Start Time', 'PMC Transaction Status', 'PMC Transaction Status Id', 'PMC Transaction Name', 'Downtime End Time', 'Downtime Total Duration', 'Unit', 'Work Order Document']
    ...
    function _post(context) {
        log.debug("PCT-POSt", context.data)
    ...
    const getOpenWorkOrders = (dataObj) => {
        ...
        log.debug("PCT-filterArray", filterArray)
        try {
            var manufacturingoperationtaskSearchObj = search.create({
                ...
                //     // "AND",
                //     // ["custrecord_pct_kfab_man_op_task.custrecord_pct_kfab_emp","anyof",dataObj.employeeId, "@NONE@"]
                // ],
                columns:
                    [
                        search.createColumn({
                        ...
                        search.createColumn({
                            name: "custbody_pct_metal_cust_po",
                            join: "workOrder",
                            summary: "GROUP",
                            label: "Customer PO#"
                        }),
                        ...
                        search.createColumn({
                            name: "custbody_pct_mt_work_order_document",
                            join: "workOrder",
                            summary: "GROUP",
            ...
            log.debug("PCT-Count", searchResultCount)
            let dataArr = [];
            if (searchResult.length) {
                for (let searchIndex = 0; searchIndex < searchResult.length; searchIndex++) {
                    ...
                    resData.customer = searchResult[searchIndex].getText({
                        name: "entity",
                        join: "workOrder",
                        summary: "GROUP",
                    })

                    resData.createdFrom = searchResult[searchIndex].getText({
                        name: "createdfrom",
                        join: "workOrder",
                        summary: "GROUP",
                    })
                    resData.customerpo = searchResult[searchIndex].getValue({
                        name: "custbody_pct_metal_cust_po",
                        join: "workOrder",
                        summary: "GROUP",
                    })
                    ...
                    resData.workOrderDocumentValue = searchResult[searchIndex].getValue({
                        name: "custbody_pct_mt_work_order_document",
                        join: "workOrder",
                        summary: "GROUP"
                    });

                    resData.workOrderDocument = searchResult[searchIndex].getText({
                        name: "custbody_pct_mt_work_order_document",
                        join: "workOrder",
                        summary: "GROUP"
                    });
                    ...
                    var fileObj = ""
                    if (resData.workOrderDocumentValue) {
                        fileObj = file.load({
                            id: searchResult[searchIndex].getValue({
                                name: "custbody_pct_mt_work_order_document",
                                join: "workOrder",
                                summary: "GROUP"
                            })
                    ...
                    resData.url = fileObj ? fileObj.url : ""

                    log.debug("PCT-PMC", resData.pmcTransactionId)
    ...
    const getOpenWorkOrderSearchCount = (dataObj) => {
        log.debug({
            title: 'PCT-getOpenWorkOrderSearchCount',
            details: JSON.stringify(dataObj)
        })
        log.debug({
            title: 'PCT-getOpenWorkOrderSearchCount',
            details:  dataObj.workCenter
        })
        let filterArray = [];
        if (dataObj.workOrder) {
            filterArray.push(["status", "anyof", "PROGRESS", "NOTSTART"])
            ...
            filterArray.push(["workorder.status", "anyof", "WorkOrd:B", "WorkOrd:D"])
            filterArray.push("AND")
            filterArray.push(["manufacturingworkcenter", "anyof", dataObj.pmcWorkCenter])
            filterArray.push("AND")
            filterArray.push(["workorder.location", "anyof", dataObj.location])
        }
        log.debug("PCT-filterArray", filterArray)
