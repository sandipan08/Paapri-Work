...
define(['N/search', 'N/record', 'N/runtime'], function (search, record, runtime) {
    ...
    function _get(context) {
        ...
        if (employeeDetailsObj.isSuccess) {
            ...
            let getWorkCenter = getWorkCenters(employeeDetailsObj.data);
            let getWorkStation = getWorkStations(employeeDetailsObj.data);
            ...
            employeeDetailsObj.data['workStation'] = getWorkStation;
            employeeDetailsObj.data['location'] = getLocation(employeeDetailsObj.data);;
        }
    ...
    const verifyEmployeeWithBadgeId = (badgeId) => {
        ...
        if (employeeCount > 0) {
            ...
            getEmployeeSearch.run().each(function (result) {
                ...
                // employeeName =employeeName + ' '+ result.getValue({ name: "altname" });
                // }
                log.debug("PCT-PMC", "employeeName : " + employeeName);
                ...
                employeeData['subsidiary'] = result.getValue({ name: "subsidiary", label: "Subsidiary" });
                employeeData['locationName'] = result.getText({ name: "location" })
                employeeData['locationId'] = result.getValue({ name: "location" }) != '' ? result.getValue({ name: "location" }) : -1
                employeeData['location'] = result.getValue({ name: "location" });
                employeeData['userRole'] = runtime.getCurrentUser().role;
                employeeData['workOrder'] = 0;
                return true;
            });
    ...
    const getWorkStations = (dataObj) => {
        ...
        var entitygroupSearchObj = search.create({
            ...
            filters:
                [
                    ...
                    /* "AND",
                     ["custentity_pct_mtc_pmc_locations", "noneof", dataObj.location]*/
                ],
            columns:
                [
        ...
        entitygroupSearchObj.run().each(function (result) {
            ...
            if (parseInt(res['internalId']) > 0) {
                workCenterArr.push(res);
            }
            return true;
        });
        log.debug("PCT-getWorkStations", JSON.stringify(workCenterArr))
        return workCenterArr;
    }
    const getWorkCenters = (dataObj) => {
        let workStationWiseCenterObj = {}
        var entitygroupSearchObj = search.create({
            type: "entitygroup",
            filters:
                [
        ...
        entitygroupSearchObj.run().each(function (result) {
            // .run().each has a limit of 4,000 results
            let res = {};
            let workStation = result.getValue({
                name: "custentity_pct_mtc_workstations_list",
                sort: search.Sort.ASC,
                label: "Work Station"
            })
            res['name'] = result.getValue({ name: "groupname" })
            // res['workStation'] = result.getValue({ name: "custentity_pct_mtc_workstations_list" })
            res['internalId'] = result.id;
            if (!(workStation in workStationWiseCenterObj)) {
                workStationWiseCenterObj[workStation] = []
            }
            workStationWiseCenterObj[workStation].push(res);
            return true;
        });
        log.debug({
            title: 'PCT-getWorkCenters',
            details: workStationWiseCenterObj
        })
        return workStationWiseCenterObj;
    }

    // Get Subsdiary Location Search 

    const getLocation = (dataObj) => {
        let locationArray = [];
        var locationSearchObj = search.create({
            type: "location",
            filters:
                [
                    // ["subsidiary", "anyof", dataObj.subsidiary],
                    // "AND",
                    ["isinactive", "is", "F"]
                ],
            columns:
        ...
        locationSearchObj.run().each(function (result) {
            ...
            res['name'] = result.getValue({ name: "name", label: "Name" })
