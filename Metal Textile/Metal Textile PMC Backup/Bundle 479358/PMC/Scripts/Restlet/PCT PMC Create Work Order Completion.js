...
define(['N/record', 'N/search', 'N/format'], function (record, search, format) {
    ...
    function _post(context) {
        try {
            ...
            const startDate = loadPmcTransaction.getValue({
                fieldId: 'custrecord_pct_kfab_res_start_date'
            })
            let downTimeDuration = loadPmcTransaction.getValue({
                fieldId: 'custrecord_pct_pmc_dwn_duration'
            })
            if (downTimeDuration == '' || downTimeDuration == null || isNaN(downTimeDuration)) {
                downTimeDuration = 0
            }
            ...
            let productionTime = ((new Date(endDate) - new Date(startDate.getTime() + downTimeDuration * 60000)) / 60000).toFixed(2)

            loadPmcTransaction.setValue({
                fieldId: 'custrecord_pct_pmc_total_time',
            ...
            const productionScrapQuantity = context.productionScrapQuantity;
            const productionScrabReason = context.screapReason;

            workOrderCompletionObj['productionScrapQuantity'] = productionScrapQuantity;
            workOrderCompletionObj['screapReason'] = productionScrabReason;
    ...
    const createWorkOrderCompletion = (dataObj) => {
        ...
        if (dataObj.screapReason != -1) {
            workOrderCompletionRecord.setValue({
                fieldId: 'custbody_pct_mtc_wc_sc_reason_code',
                value: dataObj.screapReason
            })
        }
        const result = search.lookupFields({
            type: 'manufacturingoperationtask',
            id: dataObj.operationSequenceId,
            columns: ['completedquantity']
        });
        log.debug('Completed Quantity', result.completedquantity);
        let alreadyCompletedquantity = result.completedquantity == '' ? 0 : parseInt(result.completedquantity);
        ...
        workOrderCompletionRecord.setValue({
            fieldId: 'custbody_pct_pmc_pmc_transaction',
        ...
        if (dataObj.isLastOperation && dataObj.isSerialItem && dataObj.completedQuantity > 0) {
            ...
            for (let invDetailsQuantityIndex = 0; invDetailsQuantityIndex < dataObj.completedQuantity; invDetailsQuantityIndex++) {
                ...
                log.debug("PCT-Serial",alreadyCompletedquantity + 1)
                subRecord.setSublistValue({
                    sublistId: 'inventoryassignment',
                    fieldId: 'receiptinventorynumber',
                    line: invDetailsQuantityIndex,
                    value: `${dataObj.serialNumber}-${alreadyCompletedquantity + 1}`
