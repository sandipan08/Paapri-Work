...
let restletsScriptObj = {
    ...
    'customscript_pct_mtc_res_get_scrap_reas': {
        'scriptName': 'PCT MTC RESTLET GET SCRAP REASON CODE',
        'scriptId': 'customscript_pct_mtc_res_get_scrap_reas',
        'scriptInternalId': ''
    },

    'customscript_pct_sc_get_all_work_orders': {
        'scriptName': 'PCT MTC RESTLET GET SCRAP REASON CODE',
        'scriptId': 'customscript_pct_sc_get_all_work_orders',
        'scriptInternalId': ''
    }
...
$(document).ready(async () => {
    ...
    if (loggedInUserDetails) {
        let filterDetailsHtml = `<span id='location'>Location:  ${loggedInUserDetails.locationName}</span><span id='workOrder'> Work Order: </span><span id='workStation'> Work Station: </span><span id="userWorkCenter">  Work Center:</span>`
        let basicDetailsHtml = `<span >${generateSelectField({ 'fieldId': 'locationFilter', 'fieldName': 'Location Filter', 'options': loggedInUserDetails.location })}</span> <span >${generateSelectField({ 'fieldId': 'workStationFilter', 'fieldName': 'Work Station Filter', 'options': loggedInUserDetails.workStation })}</span> <span >${generateSelectField({ 'fieldId': 'workCenterFilter', 'fieldName': 'Work Center Filter', 'options': loggedInUserDetails.workCenters })}</span>`
        // let basicDetailsHtml = `<span>Location: ${loggedInUserDetails.locationName}</span><span id='userWorkCenter'> Work Center: ${loggedInUserDetails.pmcWorkCenterName}</span> <span>${generateSelectField({ 'fieldId': 'workStationFilter', 'fieldName': 'Work Station Filter', 'options': loggedInUserDetails.workCenters })}</span> <span>${generateSelectField({ 'fieldId': 'workCenterFilter', 'fieldName': 'Work Center Filter', 'options': loggedInUserDetails.workCenters })}</span>`

        const getAllWorkOrderObj = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=${scriptInternalIdObj.customscript_pct_sc_get_all_work_orders.scriptInternalId}&deploy=1`)
        if (getAllWorkOrderObj.responseSuccess && getAllWorkOrderObj.responseData.isSuccess) {
            let workOdrerDetailsHtml = `<span >${generateDataListField({ 'fieldId': 'workOrderFilter', 'fieldName': 'Work Order Filter', 'options': getAllWorkOrderObj.responseData.data })}</span>`
            $('#secondBasicDetails').html(workOdrerDetailsHtml);
            // $('#secondFilterRegionContainers').attr('hidden', false);
        }
        // else {
        //     generateError({ 'errorMessage': getAllWorkOrderObj.responseData.errorMessage })
        // }
        ...
        $('#workStationFilter').val(-1)
        $('#locationFilter').val(loggedInUserDetails.locationId)
    ...
    /* --------------------------------------------------- Logout Function End ----------------------------------------------------------------- */
    $(document).on('change', '#mainFilter', async (e) => {
        let val = $(e.target).val()
        $('#secondBasicDetails').attr('hidden', false);
        $('#basicDetails').attr('hidden', false);
        if (parseInt(val) == 1) {
            $('#basicDetails').attr('hidden', true);
            $('#secondBasicDetails').attr('hidden', false);


        }
        else if (parseInt(val) == 2) {
            loggedInUserDetails.workOrder = 0;
            $('#secondBasicDetails').attr('hidden', true);
            $('#basicDetails').attr('hidden', false);
            $('#workOrderFilter').val('');
            // $('#workOrderFilter').val(-1)
        }
    });



    /* --------------------------------------------------- Work Order Filter Function Start --------------------------------------------------- */
    // $('#browserInput').on('change', function () {
    //     var inputValue = $(this).val();
    //     console.log("Selected value: " + inputValue);
    // });
    $(document).on('change','#workOrderFilter',async function(e){
        let workOrderId= $(this).val()
        console.log($(this).val());
        var dataId = $('#workOrdersList option').filter(function () {
            return $(this).val() === workOrderId;
          }).data('id');
          console.log(dataId);
          $('#workOrder').html(`Work Order: ${$(this).val()}`)
          loggedInUserDetails.workOrder = dataId;
          let hasDatatableGenarated = await getOpenWorkOrders()
          if (hasDatatableGenarated.isSuccess) {
              $("#openWorkOrderContainer").attr('hidden', false);
          }
    ...
    // $(document).on('change', '#workOrderFilter', async (e) => {
    //     console.log("change")
    //     let val = $(e.target).val()
    //     let workOrderText = e.target.options[e.target.selectedIndex].text;
    //     $('#workOrder').html(`Work Odrer: ${workOrderText}`)
    //     // let field;
    //     // field += `<select class="form-select workCenterFilter" id="workCenterFilter"><option value="">-Select-</option>`;
    //     // // console.log('loggedInUserDetails.workStation[val] ='+ JSON.stringify(loggedInUserDetails.workCenters))
    //     // if (loggedInUserDetails.workCenters[val] && loggedInUserDetails.workCenters[val].length > 0 && val > 0) {
    //     //     loggedInUserDetails.workCenters[val].map((element) => {
    //     //         field += `<option value=${element.internalId}>${element.name}</option>`;
    //     //     })
    //     // }
    //     // field += `</select>`;
    //     // $('#workCenterFilter').html(field)


    //     loggedInUserDetails.workOrder = val;
    //     let hasDatatableGenarated = await getOpenWorkOrders()
    //     if (hasDatatableGenarated.isSuccess) {
    //         $("#openWorkOrderContainer").attr('hidden', false);
    //     }
    ...
    // });
    /* --------------------------------------------------- Work Odrer Filter Function End --------------------------------------------------- */


    /* --------------------------------------------------- Work Station Filter Function Start --------------------------------------------------- */

    $(document).on('change', '#workStationFilter', async (e) => {
        let val = $(e.target).val()
        let workStationText = e.target.options[e.target.selectedIndex].text;
        $('#workStation').html(`Work Station: ${workStationText}`)
        let field;
        field += `<select class="form-select workCenterFilter" id="workCenterFilter"><option value="">-Select-</option>`;
        // console.log('loggedInUserDetails.workStation[val] ='+ JSON.stringify(loggedInUserDetails.workCenters))
        if (loggedInUserDetails.workCenters[val] && loggedInUserDetails.workCenters[val].length > 0 && val > 0) {
    ...
    /* --------------------------------------------------- Work Station Filter Function End --------------------------------------------------- */
    ...
    /* --------------------------------------------------- Location Filter Function Start --------------------------------------------------- */
    $(document).on('change', '#locationFilter', async (e) => {
        let location = $(e.target).val()
        let locationText = e.target.options[e.target.selectedIndex].text;
        $('#location').html(`Location: ${locationText}`)
        loggedInUserDetails.locationId = location
        $('#workCenterFilter').val(-1)
        $('#workStationFilter').val(-1)
        // let hasDatatableGenarated = await getOpenWorkOrders()
        // if (hasDatatableGenarated.isSuccess) {
    ...
    $(document).on('click', '#downTimeModalSubmit', async () => {
        if ($('#categoryType').val()) {
            ...
            generateAlert({
                'type': 'error',
                'title': 'Please Put the Value of Downtime Category',
                'message': '',
                'timer': 1500,
                'showConfirmButton': false,
                'timerProgressBar': true
...
const stopTimerClock = async () => {
    ...
    if (selectedRowData.pmcTransactionName.toLowerCase() === 'production') {
        let selectedRowWorkOrderId = selectedRowData.workOrderId;
        let productionQtyDiv = `<div><input type="number" value="" class="swal2-input" id="productionQuantity" min='0' step='0.01'><span id="swal_uom" style="font-size:40px;font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>   `

        if (workOrderAdditionalData[selectedRowWorkOrderId].isLastOperation == true) {
            const scrapReasonCode = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=${restletsScriptObj.customscript_pct_mtc_res_get_scrap_reas.scriptId}&deploy=customdeploy_pct_mtc_res_get_scrap_reas`);
            console.log("scrapReasonCode=====", scrapReasonCode);
            let getscrapreason = generateSelectField({ 'fieldId': 'getScrapeReasonCode', 'fieldName': 'Get Scrap Reason', 'options': scrapReasonCode.responseData })

            productionQtyDiv += `<div><input type="number" value="" class="swal2-input" id="productionScrapQuantity" min='0' step='0.01' placeholder="Scrap Qty"><span id="swal_uom" style="font-size:40px;font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`
            productionQtyDiv += `<div>${getscrapreason}</div>`
        }
        enableUI();
        if (int)
            clearInterval(int);
        Swal.fire({
            ...
            html: productionQtyDiv,//`<div><input type="number" value="" class="swal2-input" id="productionQuantity" min='0' step='0.01'><span id="swal_uom" style="font-size:40px;font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>   <div><input type="number" value="" class="swal2-input" id="productionScrapQuantity" min='0' step='0.01' placeholder="scrap Qty"><span id="swal_uom" style="font-size:40px;font-weight: bold;">${selectedRowData.workOrderUnit}</span></div>`,
            width: 700,
            confirmButtonText: 'Ok',
            focusConfirm: false,
            preConfirm: async () => {
                const productionQuantity = Swal.getPopup().querySelector('#productionQuantity').value
                let productionScrabReason = ""
                let productionScrapQuantity = 0

                if (workOrderAdditionalData[selectedRowWorkOrderId].isLastOperation === true) {
                    productionScrapQuantity = Swal.getPopup().querySelector('#productionScrapQuantity',).value
                    productionScrabReason = Swal.getPopup().querySelector('#getScrapeReasonCode').value
                }

                if (!productionQuantity) {
                    Swal.showValidationMessage(`Please enter Production Quantity`)
                }
                return { productionQuantity: productionQuantity, productionScrapQuantity: productionScrapQuantity, screapReason: productionScrabReason }
            }
        }).then(async (result) => {
            if (result.isConfirmed) {

                let qtyEntered = result.value.productionQuantity
                let productionScrapQuantity = result.value.productionScrapQuantity
                ...
                completionObj['screapReason'] = result.value.screapReason
                console.log(completionObj);
...
const getOpenWorkOrders = async () => {
    ...
    let data = {
        ...
        'location': loggedInUserDetails.locationId,
        'employeeName': loggedInUserDetails.employeeName,
        'workOrder': loggedInUserDetails.workOrder
    ...
    openWorkOrderDataTable = $('#openWorkOrderTable').DataTable({
        ...
        columnDefs: [
            //hide the second & fourth column
            { 'visible': false, 'targets': [1, 3, 5, 6, 9, 20, 22, 26] },
        ],
        initComplete: function (settings, json) {
    ...
    openWorkOrderDataTable.off('select').on('select', async () => {
        ...
        let informationObj = {
            ...
            'startTime': {
                ...
                'fieldId': 'startTime',
                'fieldValue':  selectedRowData.pmcTransactionResultStartTime//selectedRowData.pmcTransactionStatusId === '3' ? selectedRowData.pmcTransactionResultStartTime : ''
            },
            'workOrderPlanedQty': {
                'fieldName': 'Work Order Qty',
            ...
            'customer': {
                'fieldName': 'Customer',
                'fieldId': 'customer',
                'fieldValue': selectedRowData.customer
            },

            'createdFrom': {
                'fieldName': 'SO#',
                'fieldId': 'createdFrom',
                'fieldValue': selectedRowData.createdFrom
            },

            'customerpo': {
                'fieldName': 'PO#',
                'fieldId': 'customerpo',
                'fieldValue': selectedRowData.customerpo
            },
            ...
            'workOrderDocument': {
                'fieldName': 'Work Order Document',
                'fieldId': 'custbody_pct_mt_work_order_document',
                'fieldValue': `<a href="${selectedRowData.url}">${selectedRowData.workOrderDocument}</a>`
            }
        }
        if (selectedRowData.createdFrom == '- None -') {
            delete informationObj.customerpo;
            delete informationObj.customer;
            delete informationObj.createdFrom;
        }
...
const generateDataListField = (dataObj) => {
    let field = '';
    field += dataObj.fieldName ? `<label for=${dataObj.fieldId} class="col-form-label">${dataObj.fieldName} </label>` : '';
    // field += `<select class="form-select ${dataObj.fieldId}" id=${dataObj.fieldId}><option value="-1">-Select-</option>`;
    field += ` <input class="form-select ${dataObj.fieldId}" list="workOrdersList" id="${dataObj.fieldId}" placeholder="Start typing..."><datalist id="workOrdersList">`;
    if (dataObj.options && dataObj.options.length > 0) {
        dataObj.options.map((element) => {
            // field += `<option value=${element.internalId}>${element.name}</option>`;
            field += `  <option value=${element.name} data-id=${element.internalId}>`;
        })
    }
    field += `  </datalist>`;
    return field;
