...
function CreateToolsIRorIA() {
    nlapiLogExecution('DEBUG', 'PCT-Log', 'IN SCRIPT');
        ...
        for (var k = 1; k <= subrecord.getLineItemCount('inventoryassignment'); k++) {
            ...
            if (searchresults != null)
                ...
                for (var i = 0; searchresults != null && i < searchresults.length; i++) {
                    ...
                    // nlapiLogExecution('DEBUG', 'PCT-Log', 'Serialized Tool Result Count: ' + serializedToolCount(strSerial));
                    if (serializedToolCount(strSerial)) {
...
function serializedToolCount(strSerial) {
    nlapiLogExecution('DEBUG', 'PCT-Log', 'Serialized Tool  : ' + strSerial);
    // var serialToolCount = 0;
    var customrecord_pct_toolSearch = nlapiSearchRecord("customrecord_pct_tool", null,
        [
    ...
    nlapiLogExecution('DEBUG', 'PCT-Log', 'Serialized Tool Result : ' + customrecord_pct_toolSearch);
    // nlapiLogExecution('DEBUG', 'PCT-Log', 'Serialized Tool Count : ' + customrecord_pct_toolSearch.length);
    // serialToolCount = customrecord_pct_toolSearch.length;
    if (customrecord_pct_toolSearch == null) {
        return true;
    }
    else {
