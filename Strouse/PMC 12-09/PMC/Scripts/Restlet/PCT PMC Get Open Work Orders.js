...
define(['N/search'], function (search) {
        ...
        try {
            ...
            if (searchResult.length) {
                for (let searchIndex = 0; searchIndex < searchResult.length; searchIndex++) {
                    ...
                    if (searchResult[searchIndex].getValue({
                        name: "runrate",
                        summary: "GROUP"
                    }) != 0) {
                        resData.runRateHour = (60 / searchResult[searchIndex].getValue({
                            name: "runrate",
                            summary: "GROUP"
                        })).toFixed(2)
                    }
                    else { resData.runRateHour = 0 }
                    ...
                    resData.notes = getAssemblyNotes(searchResult[searchIndex].getValue({
                        name: "item",
                        join: "workOrder",
                        summary: "GROUP"
                    }));
                    resData.estimatedSetupTime = searchResult[searchIndex].getValue({
                        name: "setuptime",
                        summary: "GROUP",
                        label: "Setup Time (Min)"
                    });
    ...
    const getAssemblyNotes = (itemId) => {
        log.debug("PCT", "In Get Assembly Notes Item : " + itemId)
        ...
        assemblyitemSearchObj.run().each(function (result) {
            ...
            let notesId = result.getValue({
                name: "internalid",
                join: "userNotes",
                label:
                    "Internal ID"
            })
            html += `<li><a href="/app/crm/common/note.nl?id=${notesId}" target="_blank"> ${result.getValue({
                name: "title",
                join: "userNotes",
                label: "Title"
            })}</a></li>`;
            //  notesObjArray.push(  `<li><a href="/app/crm/common/note.nl?id=${notesId}&e=T&l=T" target="_blank"> ${result.getValue({
            //     name: "title",
            //     join: "userNotes",
            //     label: "Title"
            //  })}</a></li>`)
        ...
        html += '</ol> ';
        log.debug("PCT", "notesObjArray : " + notesObjArray)
        return html;
