...
$(document).ready(async () => {
    ...
    $('#refreshButton').click(async () => {
        ...
        let recordId = $('#customReportRecordId').val();
        getVarianceReportLink = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=customscript_pct_get_report_link&deploy=customdeploy_pct_get_report_link&recordId=${recordId}`)
        console.log(getVarianceReportLink)
        if (getVarianceReportLink.responseSuccess && getVarianceReportLink.responseData.isSuccess) {
            console.log(getVarianceReportLink.responseData.data);
            $("#refreshButton").attr("hidden", true);
            $('#viewReportButton').removeAttr('hidden');
        }
        else if (getVarianceReportLink.responseData.data == 'Report is creating...') {
            creatingReport({ 'warningMessage': getVarianceReportLink.responseData.data })
        }
        else {
            generateError({ 'errorMessage': getVarianceReportLink.responseData.data })
        }
    ...
    $('#viewReportButton').click(function () {
        $("#refreshButton").attr("hidden", true);
        window.open(getVarianceReportLink.responseData.data, "_self");
        // $("#refreshButton").attr('disabled','disabled');
    });

    $('#toDate').change(async function () {
        if (!$('#fromDate').val()) {
            alert('From Date can not be blank');
            // $('#fromDate').prop('autofocus');
    ...
    $('#filterType').change(async function () {
        ...
        if (selectFilter == 1) {
            ...
            $("#fromDateFilter").attr("hidden", true);
            $("#toDateFilter").attr("hidden", true);
            $("#sceDiv").attr("hidden", false);
            ...
            $('#fromDateFilter').removeAttr('hidden');
            $('#toDateFilter').removeAttr('hidden');
...
//         const getWoFromItemObj = await fetchGetRequest(`/app/site/hosting/restlet.nl?script=customscript_pct_wov_get_wo_from_item&deploy=customdeploy_pct_wov_get_wo_from_item&selectItem=${selectItem}`)
...
//     let summaryTableReport = $('#summaryTableReport').DataTable({
//         "pageLength": 50,
//         order: [[0, 'asc']],
//         "retrieve": true,
//         'paging': false,
...
const creatingReport = (dataObj) => {
    swalAlertObj.type = 'warning';
    swalAlertObj.title = dataObj.warningMessage;
    swalAlertObj.timer = dataObj.timer ? dataObj.timer : 2000;
    swalAlertObj.timerProgressBar = true;
    swalAlertObj.showConfirmButton = dataObj.showConfirmButton ? dataObj.showConfirmButton : false
    generateAlert(swalAlertObj)
}