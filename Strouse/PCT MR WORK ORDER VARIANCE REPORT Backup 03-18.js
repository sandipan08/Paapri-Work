    ...
    function (record, log, search, runtime, file, formati) {
        ...
        function reduce(context) {
            ...
            try {
                ...
                let sceId=0, sceQty = 0, sceDocumentNumber = ''

                if (workOrderObj.getValue('custbody_pct_config_number_in_wo')) {
                    sceId = workOrderObj.getValue('custbody_pct_config_number_in_wo')
                    sceDocumentNumber = workOrderObj.getText('custbody_pct_config_number_in_wo')
                    sceQty = search.lookupFields({
                ...
                // log.debug("PCT - ActualDataFromWOCompletion", ActualDataFromWOCompletion)
        ...
        function summarize(context) {
            ...
            values.forEach((element, index) => {
                ...
                summaryObj['totalActualCost'] = convertCurrency(parseFloat(totalActualCost).toFixed(2))
                summaryObj['totalPlannedCost'] = convertCurrency(parseFloat(totalPlannedCost).toFixed(2))
            ...
            // log.debug("PCT_customReocrdId", varianceReportId)
            ...
            if (customReocrdId) {
                ...
                record.load({
                    ...
                    id: runtime.getCurrentScript().getParameter({ name: 'custscript_pct_wo_variance_report_id' }),
                    isDynamic: true
                }).setValue({
        ...
        const updateCustomRecord = (customReocrdId, fileId) => {
            return workOrderVarianceDataStoreRecordId = record.load({
                ...
                value: `https://4344933.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1210&deploy=1&compid=4344933&fileData=&customRecordId=${customReocrdId}&custparam_success=1&whence=`,
                ignoreFieldChange: true
            }).save();
        ...
        const getEstimatedItemDetails = (workOrderArray, sceId, sceDocumentNumber, sceQty, workOrderNumber, workOrderQty) => {
            ...
            do {
                ...
                for (let woIndex = 0; woIndex < result.length; woIndex++) {
                    ...
                    itemObj.estimatedQuantity = ((itemEstimatedQuantity / workOrderQuantity) * builtQuantity).toFixed(2);
                    ...
                    itemObj.sceEstimatedMaterialCost = (Object.keys(sceDetailsResponse).length) ? (CheckNull(sceDetailsResponse.sceEstimatedMaterialCost)).toFixed(2) : 0
            ...
            for (let index = 0; index < sceItemArray.length; index++) {
                if (!getAbsentItemInWorkOrder(sceItemArray[index], workOrderArray)) {
                    ...
                    workOrderDetailObj[workOrderArray][sceItemArray[index]] = {
                        ...
                        "sceEstimatedMaterialCost": (Object.keys(sceDetailsResponse).length) ? (CheckNull(sceDetailsResponse.sceEstimatedMaterialCost)).toFixed(2) : 0,
                        "averageCost": 0,
                        ...
                        "actualQuantity": 0,
                        "differentiateQuantity": (Object.keys(sceDetailsResponse).length) ? sceDetailsResponse.sceEstimatedQuantity : 0,
                        "actualItemCost": 0,
                        "actualItemCostCurrency": convertCurrency(0),
                        "differentiateItemCost": 0,
                        "differentiateItemCostCurrency": convertCurrency((Object.keys(sceDetailsResponse).length) ? (sceDetailsResponse.sceEstimatedMaterialCost).toFixed(2) : 0),
                        "color": 'green'
                    }
                }
        ...
        const getActualItemDetails = (workOrderEstimatedItemDataObj, workOrderArray) => {
            ...
            getActualItemDetailsFilterArray.forEach(element => {
                ...
                if (obj && element.itemId in obj) {
                    ...
                    obj[element.itemId]['differentiateQuantity'] = (parseFloat(obj[element.itemId]['sceEstimatedQuantity']) - parseFloat(element.actualQuantity)).toFixed(2);
        ...
        const getOperationData = (workOrderId, EstimateDataFromMFGOperationTask, ActualDataFromWOCompletion, sceId, sceDocumentNumber, workOrderNumber) => {
            ...
            if (estimatedDataObj != null) {
                ...
                Object.keys(estimatedDataObj).forEach(function (key) {
                    ...
                    log.debug("PCT-operationWiseActualDataArray", operationWiseActualDataArray)
                ...
                sceProcessArray.map((element) => {
                    if (!getAbsentProcessInWorkOrder(element.sceProcessName, element.operationSequence, workOrderId, element.workCenter)) {
                        // log.debug("PCT", "In Absent Process Loop")
        ...
        const getEstimateDataFromMFGOperationTask = (workOrderArray, costCategoryObj, sceId, sceDocumentNumber) => {
            ...
            manufacturingoperationtaskSearchObj.run().each(function (result) {
                ...
                operationObj.estRunTime = (parseFloat(result.getValue({ name: "runrate", label: "Run Rate (Min/Unit)" })) * result.getValue({ name: "completedquantity", label: "completedquantity" })).toFixed(2)
            ...
            log.debug("PCT", "getEstimateDataFromMFGOperationTask Object : " + JSON.stringify(operationNameObj));
            ...
            if (searchResultCount > 0) {
                customrecord_pct_itemsSearchObj.run().each(function (result) {
                    ...
                    sceObject['sceEstimatedQuantity'] = (result.getValue({
                        ...
                        summary: "GROUP",
                        label: "Order Qty"
                    }) * result.getValue({ name: "custrecord_pct_cpqitem_total_qty_perunit", summary: "SUM", label: "Total Quantity per Unit" })).toFixed(2)
            ...
            if (searchResultCount > 0) {
                customrecord_pct_processesSearchObj.run().each(function (result) {
                    sceProcessObject['estimatedTotalRunTime'] = (result.getValue({
                        ...
                        join: "CUSTRECORD_PCT_CPQ_LINKPCTCONFPROCESTEPS",
                        label: "Order Qty"
                    }) * result.getValue({ name: "custrecord_pct_cpq_total_time_in_min", label: "Time per Unit(in Min)" })).toFixed(2);
                    sceProcessObject['estimatedTotalSetupTime'] = parseFloat(result.getValue({ name: "custrecord_pct_sc_setup_time_inmin", label: "Setup Time (In Min)" })).toFixed(2);
                    sceProcessObject['estimatedTotalProcessCost'] = parseFloat(result.getValue({ name: "custrecord_pct_config_process_total_cost", label: "Total Process Cost" })).toFixed(2)
                    return true;
                });
            }
        ...
        const getAbsentProcessInWorkOrder = (operationName, operationSequence, workOrderId, workCenter) => {
            log.debug("PCT-getAbsentProcessInWorkOrder", "operationName : " + operationName + " ,operationSequence : " + operationSequence + " ,workOrderId : " + workOrderId + " ,workCenter : " + workCenter)
            ...
            var workorderSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        "AND",
                        ["manufacturingoperationtask.manufacturingworkcenter", "anyof", workCenter]
                    ],
                columns:
                    [
