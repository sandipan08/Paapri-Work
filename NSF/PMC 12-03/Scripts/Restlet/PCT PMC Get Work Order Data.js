...
define(['N/record', 'N/search'], function (record, search) {
    ...
    function _get(context) {
      try {
            ...
            let PMCworkInstructionDetails = getWorkInstructionData(workOrderId);
            let workInstructionDetails = getPMCWorkInstructionData(workOrderId);
    ...
    const getManufacturingOperationTaskData = (workOrderId) => {
        ...
        try {
            ...
            if (searchResultCount > 0) {
                ...
                log.debug("PCT", JSON.stringify(manufacturingOperationTaskDataArr))
    ...
    const getPMCTransactionDetails = (workOrderId, operationTaskId) => {
        ...
        try {
            var customrecord_pct_pmc_transactionSearchObj = search.create({
                ...
                filters:
                    [
                        ["custrecord_pct_pmc_work_order", "anyof", workOrderId],
                        "AND",
                        ["custrecord_pct_pmc_manufacturing_task_id", "anyof", operationTaskId]
                    ],
                columns:
                    [
                        search.createColumn({
                        ...
                        search.createColumn({ name: "custrecord_pct_pmc_operation_sequence", label: "Operation Sequence" }),
                        search.createColumn({ name: "custrecord_pct_pmc_manufacturing_task_id", label: "Manufacturing Operation Task" }),
                        search.createColumn({ name: "custrecord_pct_pmc_work_center", label: "Work Center" }),
                        search.createColumn({ name: "custrecord_pct_pmc_work_order", label: "Work Order" }),
                        search.createColumn({ name: "custrecord_pct_pmc_employee", label: "Employee" }),
                        search.createColumn({ name: "custrecord_pct_pmc_res_start_date_time", label: "Result Start Date Time" }),
                        search.createColumn({ name: "custrecord_pct_pmc_res_end_date_time", label: "Result End Date Time" }),
                        search.createColumn({ name: "custrecord_pct_pmc_production_quantity", label: "Production Quantity" }),
                        search.createColumn({ name: "custrecord_pct_pmc_completion_number", label: "Completion Number" }),
                        search.createColumn({ name: "custrecord_pct_pmc_operation_status", label: "Operation Status" }),
            ...
            var searchResultCount = customrecord_pct_pmc_transactionSearchObj.runPaged().count;
            log.debug("customrecord_pct_pmc_transactionSearchObj result count", searchResultCount);
            if (searchResultCount > 0) {
                customrecord_pct_pmc_transactionSearchObj.run().each(function (result) {
                    ...
                    res.operationTask = result.getText({ name: "custrecord_pct_pmc_manufacturing_task_id" })
                    res.workCneter = result.getText({ name: "custrecord_pct_pmc_work_center" })
                    res.workOrder = result.getText({ name: "custrecord_pct_pmc_work_order" })
                    res.employee = result.getText({ name: "custrecord_pct_pmc_employee" })
                    res.resultStartDate = result.getValue({ name: "custrecord_pct_pmc_res_start_date_time" })
                    res.resultEndDate = result.getValue({ name: "custrecord_pct_pmc_res_end_date_time" })
                    res.productionQuantity = result.getValue({ name: "custrecord_pct_pmc_production_quantity" })
                    res.completionNumber = result.getText({ name: "custrecord_pct_pmc_completion_number" })
                    res.operationStatus = result.getText({ name: "custrecord_pct_pmc_operation_status" })
                ...
                log.debug("PCT", pmcTransactionArr)
    ...
    const getWorkInstructionData = (workOrderId) => {
        let keyObjects = {
            'operationSequence': '',
            // 'operationName': '',
            // 'WorkCenter': '',
        ...
        try {
            var customrecord_pct_pmc_work_instructionSearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        // search.createColumn({ name: "custrecord_pct_str_operation_name_wo", label: "OPERATION NAME" }),
                        // search.createColumn({ name: "custrecord_pct_str_mfg_work_center_wo", label: "WORK CENTER" }),
            ...
            if (searchResultCount > 0) {
                customrecord_pct_pmc_work_instructionSearchObj.run().each(function (result) {
                    ...
                    // res.operationName = result.getValue({ name: "custrecord_pct_str_operation_name_wo" })
                    // res.WorkCenter = result.getText({ name: "custrecord_pct_str_mfg_work_center_wo" })
    ...
    const getTableHeaders = () => {
        return {
            'itemsTableTitleArr': ['Item', 'Item Id', 'Total Qty on Hold', 'Total Qty Issued to this WO', 'Remaining Qty', 'Back Ordered', 'Component Yield', 'Qty with Zero Waste', 'Unit of Measure', 'Line No'],
