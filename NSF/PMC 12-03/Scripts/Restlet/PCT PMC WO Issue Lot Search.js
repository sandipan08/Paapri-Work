...
define(['N/search'], function (search) {
	...
	function _get(context) {
		...
		else if (context.searchType == "findSerializedNumbersWithBinNumber") {
			let itemSearchResponse = findSerializedNumbersWithBinNumber(context);
			return { 'isSuccess': true, 'data': itemSearchResponse.data }
		}
		return { 'isSuccess': false, 'errorMessage': 'No Data Found' }
	}

	const findSerializedNumbersWithBinNumber = (dataObj) => {
		...
		if (searchResultCount > 0) {
			...
			inventorynumberSearchObj.run().each(function (result) {
				let itemObj = {};
				itemObj['name'] = result.getValue({ name: "inventorynumber", label: "Number" });
				itemObj['internalId'] = result.id;
				itemObj['quantityOnHand'] = result.getValue({ name: "quantityonhand", label: "On Hand" });
	...
	// ----------------------------- Item Bin Number with serialized Number Search Function End ----------------------------------
