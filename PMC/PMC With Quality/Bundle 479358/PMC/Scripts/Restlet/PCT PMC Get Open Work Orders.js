...
define(['N/search'], function (search) {
    ...
    const getOpenWorkOrders = (dataObj) => {
        ...
        try {
            var manufacturingoperationtaskSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        // "AND",
                        // ["custrecord_pct_pmc_manufacturing_task_id.custrecord_pct_pmc_employee","anyof",dataObj.employeeId, "@NONE@"]
                    ],
                columns:
                    [
                        search.createColumn({
                        ...
                        search.createColumn({
                            name: "internalid",
                            join: "custrecord_pct_pmc_manufacturing_task_id",
                            summary: "MAX",
                            label: "Internal ID"
                        }),
                        search.createColumn({
                            name: "formulanumeric",
                            summary: "MAX",
                            formula: `CASE  WHEN {custrecord_pct_pmc_manufacturing_task_id.custrecord_pct_pmc_employee} = '${dataObj.employeeName}' THEN {custrecord_pct_pmc_manufacturing_task_id.internalid} ELSE 0 END`,
            ...
            log.debug("manufacturingoperationtaskSearchObj result count", searchResultCount +' ${dataObj.employeeName} ='+dataObj.employeeName);
            ...
            if (searchResult.length) {
                for (let searchIndex = 0; searchIndex < searchResult.length; searchIndex++) {
                    ...
                    resData.operationName = searchResult[searchIndex].getValue({
                        name: "name",
                        summary: "GROUP"
                    })
                    ...
                    log.debug("PCT-Employee",dataObj.employeeName)
                    ...
                    resData.workOrderUnit = searchResult[searchIndex].getText({
                        name: "unit",
                        join: "workOrder",
                        summary: "GROUP"
                    });

                    log.debug("PCT-PMCID", resData.pmcTransactionId)
                    if (resData.pmcTransactionId) {
                        let pmcTransactionData = search.lookupFields({
                            ...
                            columns: ['custrecord_pct_pmc_employee', 'custrecord_pct_pmc_operation_status', 'custrecord_pct_pmc_res_start_date_time', 'custrecord_pct_pmc_dwn_start_time', 'name', 'custrecord_pct_pmc_dwn_end_time', 'custrecord_pct_pmc_dwn_duration']
                        })
                        log.debug({
                            title: 'PCT-PMC',
                            details: `PMC Transaction Data = ${JSON.stringify(pmcTransactionData)}`
                        })
                        resData.pmcTransactionStatus = pmcTransactionData.custrecord_pct_pmc_operation_status[0].text;
                        resData.pmcTransactionStatusId = pmcTransactionData.custrecord_pct_pmc_operation_status[0].value;
                        resData.pmcTransactionResultStartTime = pmcTransactionData.custrecord_pct_pmc_res_start_date_time;
                ...
                log.debug("PCT", JSON.stringify(dataArr))
