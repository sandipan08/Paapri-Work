...
define(['N/file', 'N/render', 'N/search', 'N/log', 'N/redirect', 'N/url', 'N/record', 'N/format', 'N/runtime'],
    function (file, render, search, log, redirect, url, record, format, runtime) {
        ...
        function onRequest(context) {
            ...
            if (_request.method == 'GET') {
                ...
                var ticketNumberNew = _request.parameters.custparam_ticketNumberNew;

                // Getting the external url of all pages.
                var logoutPageUrl = GetLogoutPageUrl();
                var checkTicketPageUrl = GetCheckTickettPageUrl();
                ...
                if (backFromBreak == "BREAK_COMPLETE") {
                    ...
                    if (mode == "VIEW_MODE" || mode == "START_MODE") {
                        ...
                        if (mode == "VIEW_MODE") {
                            ...
                            // Go to the Home Page
                            // NOte: Mode = "VIEW_MORE"
                            PageRendererByMode(userId, userName, todayQuantity, loginRecordId, 0, logoutPageUrl, checkTicketPageUrl,
                                breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew,
                        ...
                        if (mode == "START_MODE") {
                            ...
                            PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, checkTicketPageUrl,
                                breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, "START_MODE",
                        ...
                        PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl, homePageUrl,
                            woCompletionUrl, qrCode, ticketNumberNew, 'HOME_PAGE', getOperationStatus, COMPLETION_RECORD_ID);
                    }
                ...
                var setupProduction = _request.parameters.setupProduction;
                ...
                if (Break_end == 'Break_end') {
                    ...
                    var mfgOperationSearchObj = GetManufacturingOperationDetails(operationInternalId, operationNumber);
                    PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl,
                        homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, "BREAK_END_MODE", mfgOperationSearchObj);//"BREAK_END_MODE",
                }
                else {
                    ...
                    if (mfgOperationSearchObj.isSuccess) {
                        ...
                        if (status == "COMPLETE") {
                            ...
                            PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl,
                                homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, status, '');
                            ...
                            if (mode == "VIEW_MODE") {
                                // Go to the Home Page
                                // NOte: Mode = "VIEW_MORE"
                                PageRendererByMode(userId, userName, todayQuantity, loginRecordId, 0, logoutPageUrl, checkTicketPageUrl, breakPageUrl,
                                    homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, "VIEW_MODE", mfgOperationSearchObj);

                            } else { // START_MODE
                                // Creating PMC Transaction table for bundle start
                                var startRecordId = CreateTransactionRecord(userId, "Running", operationInternalId, operationNumber, mfgOperationSearchObj, setupProduction);//"Bundle Start"//In Process
                                // Go to the Home Page
                                // NOte: Mode = "START_MODE"
                                PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, checkTicketPageUrl,
                                    breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew,
                        ...
                        // NOte: Mode = "START_MODE"
                        PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl, homePageUrl,
                            woCompletionUrl, ticketNumber, ticketNumberNew, _status, getOperationStatus);
                    }
         ...
         * @param {string} status 
         */
        function PageRenderer(userId, userName, todayQuantity, loginRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, status, getOperationStatus, COMPLETION_RECORD_ID) {

            var faviconUrl = GetFaviconImgUrl();
            // let downTimeResponse = getDownTimeReasonCategory()
            // if (downTimeResponse.isSuccess) {
            //     var downTimeReason = downTimeResponse.downTimeCategoryReason
            //     var downTimeCategory = downTimeResponse.downTimeCategoryArray
            // }
            // else {
            //     var downTimeReason = downTimeResponse.downTimeCategoryReason
            //     var downTimeCategory = downTimeResponse.downTimeCategoryArray
            ...
            var dataSource = {
                ...
                checkTicketPageUrl: checkTicketPageUrl,
         ...
         */
        function PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, checkTicketPageUrl, breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, ticketNumberNew, mode, dataObj) {
            ...
            var setupProductionType = checkOperationStatusObj.name
            ...
            var dataSource = {
                ...
                startTime: search.lookupFields({
                    type: 'customrecord_pct_pmc_tran_k_fab',
                    id: startRecordId,
                    columns: 'custrecord_pct_kfab_res_start_date'
                ...
                setupProductionType: setupProductionType,
                expShipDate: dataObj.expShipDate,
        ...
        const getWorkInstruction = (seq, workorderId) => {
            var customrecord_pct_pmc_instructionSearchObj = search.create({
                ...
                columns:
                    [
                        // search.createColumn({ name: "custrecord_pct_pct_ins_item", label: "Assembly Item" }),
        ...
        const getDownTimeReasonCategory = () => {
            var customrecord_pct_pmc_dwn_reasonSearchObj = search.create({
            ...
            if (searchResultCount > 0) {
                ...
                customrecord_pct_pmc_dwn_reasonSearchObj.run().each(function (result) {
                    ...
                    let downCategory = result.getValue({ name: "custrecord_pct_pmc_dwn_category", label: "Category" })
                    downTimeReasonObj['internalId'] = result.id;
                    downTimeReasonObj['name'] = result.getValue({
                        name: "name",
                        label: "Name"
                    })
                    downTimeReasonObj['categoryId'] = result.getValue({ name: "custrecord_pct_pmc_dwn_category", label: "Category" })
                    downTimeReasonObj['categoryName'] = result.getText({ name: "custrecord_pct_pmc_dwn_category", label: "Category" })
                    downTimeCategoryArray.push({
                        'internalId': result.getValue({ name: "custrecord_pct_pmc_dwn_category", label: "Category" }),
                        'name': result.getText({ name: "custrecord_pct_pmc_dwn_category", label: "Category" }),
});
                ...
                log.debug("PCT-Sandipan", JSON.stringify(downTimeCategoryObj))
                log.debug("PCT-Sandipan-downTimeCategoryArray", downTimeCategoryArray)
                return { 'isSuccess': true, 'downTimeCategoryArray': downTimeCategoryArray, 'downTimeCategoryReason': downTimeCategoryObj }
            }
            return { 'isSuccess': false, 'downTimeCategoryArray': [], 'downTimeCategoryReason': {} }
        ...
        function GetTodayUnitCount(userId) {
            ...
            if (searchResultCount > 0) {
                searchObject.run().each(function (result) {
                    // .run().each has a limit of 4,000 results
                    let qty = result.getValue({ name: "custrecord_pct_kfab_prod_qty", label: "Quantity" })
                    if (isNaN(qty) || qty == '' || qty == null || qty == undefined) {
                        qty = 0
                    }
     ...
     */
        ...
        function GetManufacturingOperationDetails(internalId, sequence) {
            ...
            var mfgOpTaskSearchObj = search.create({
                ...
                columns:
                    [//
                        ...
                        search.createColumn({ name: "item", join: "workOrder", label: "Item" }),
                        search.createColumn({ name: "custbody_acs_expected_ship_date", join: "workOrder", label: "Exp Ship Date" }),
                        search.createColumn({ name: "enddate", join: "workOrder", label: "End Date" }),
                        search.createColumn({ name: "entity", join: "workOrder", label: "entity" }),
                        search.createColumn({
                            name: "tranid",
                            join: "workOrder",
            ...
            if (searchResultCount > 0) {
                mfgOpTaskSearchObj.run().each(function (result) {
                    ...
                    searchResults.customer = result.getText({ name: "entity", join: "workOrder", label: "customer" });
                    searchResults.expShipDate = result.getValue({ name: "custbody_acs_expected_ship_date", join: "workOrder", label: "Exp Ship Date" });
                    if (searchResults.expShipDate == '' || searchResults.expShipDate == null) {
                        searchResults.expShipDate = result.getValue({ name: "enddate", join: "workOrder", label: "END Date" });
                    }
         ...
         */
        function CreateTransactionRecord(userId, actionType, operationInternalId, operationNumber, mfgOperationSearchObj, setupProduction) {
            var now = new Date();
            ...
            var recordId = record.create({
                ...
                fieldId: 'custrecord_pct_kfab_res_start_date_wos',
                ...
                fieldId: "name",
                value: setupProduction/*'Production'*/,
                //ignoreFieldChange: true
            }).save({
                enableSourcing: true,
        ...
        function UpdateBreakTransactionRecord(recordId) {
            ...
            if (downtimeStart != null && downtimeStart != '') {
                ...
                customRecord.setValue({
                    fieldId: 'custrecord_pct_kfab_dwn_start_time',
                    value: '',
                    //ignoreFieldChange: true
                }).setValue({
                    fieldId: 'custrecord_pct_kfab_dwn_duration',
                    value: durationInMin,
                    //ignoreFieldChange: true
                }).setValue({
                    fieldId: 'custrecord_pct_kfab_op_status',
            ...
            customRecord.save();
            //custrecord_pct_kfab_downlime_category
        ...
        //
        function GetCheckTickettPageUrl() {
            return url.resolveScript({
                scriptId: 'customscript_pct_pmc_lite_cehck_vad_tkt',
                deploymentId: 'customdeploy_pct_pmc_lite_cehck_vad_tkt',
                returnExternalUrl: true
            });
        }
        /**
        ...
        */
        function checkOperationStatus(userId, operationId) {
            ...
            customrecord_pct_pmc_tran_k_fabSearchObj.run().each(function (result) {
                ...
                checkOperationStatusObj.name = result.getValue('name');
