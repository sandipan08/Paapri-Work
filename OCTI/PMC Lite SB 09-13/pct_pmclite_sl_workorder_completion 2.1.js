    ...
    function (file, render, search, log, redirect, url, record, format, error, runtime) {
        ...
        function onRequest(context) {
            ...
            if (_request.method == 'GET') {
                try {
                    ...
                    var setupProduction = _request.parameters.custparam_setupProduction;
                    var now = new Date();
                    ...
                    var totalDuration = dataObject.labor_differenceInMinutes
                    ...
                    if (getMFGTaskDetail.status != 'COMPLETE') {
                        ...
                        if (setupProduction == 'Production') {
                            ...
                            //recordsetup,machinesetuptime,laborsetuptime,machineruntime,laborruntime
                        } else {
                            transformRecordObject.setCurrentSublistValue({
                                sublistId: 'operation',
                                fieldId: 'recordsetup',
        ...
        const getTotalDurationObj = (pmcTransactionId, now) => {
            ...
            let labor_start_date_time = loadPmcTransaction.getText({
                fieldId: 'custrecord_pct_kfab_res_start_date'//custrecord_pct_kfab_res_start_date
            })
        ...
        */
        ...
        function get_time_set(labor_start_date_time, labor_end_date_time, emp) {
            ...
            var customrecord_pct_pmc_tran_k_fabSearchObj = search.create({
                ...
                filters:
                    [
                        ["custrecord_pct_kfab_res_start_date", "notonorafter", end],
                        "AND",
                        [["custrecord_pct_kfab_res_end_date", "notonorbefore", start], "OR", ["custrecord_pct_kfab_res_end_date", "isempty", ""]],
                        "AND",
                        ["custrecord_pct_kfab_wo", "noneof", "@NONE@"],
                        "AND",
                        ["custrecord_pct_kfab_emp", "anyof", user],

                    ],
                columns:
                    [

                        search.createColumn({ name: "custrecord_pct_kfab_res_start_date", label: "start date" }),
                        search.createColumn({ name: "custrecord_pct_kfab_res_end_date", label: "end date" }),
            ...
            log.debug("customrecord_pct_pmc_tran_k_fabSearchObj result count", searchResultCount);
            customrecord_pct_pmc_tran_k_fabSearchObj.run().each(function (result) {
                if (result.getValue("custrecord_pct_kfab_res_start_date") != null && result.getValue("custrecord_pct_kfab_res_start_date") != "") {
                    time_set[set_index] = result.getValue("custrecord_pct_kfab_res_start_date");
                    set_index++;
                }
                if (result.getValue("custrecord_pct_kfab_res_end_date") != null && result.getValue("custrecord_pct_kfab_res_end_date") != "") {
                    time_set[set_index] = result.getValue("custrecord_pct_kfab_res_end_date");
                    set_index++;
                }
                return true;
            });
            ...
            var customrecord_pct_pmc_tran_k_fabSearchObj = search.create({
                ...
                filters:
                    [
                        ["name", "is", "Setup"],
                        "AND",
                        ["custrecord_pct_kfab_res_start_date", "notonorafter", start_setup],
                        "AND",// 23/05/2024
                        [["custrecord_pct_kfab_res_end_date", "notbefore", end_setup], "OR", ["custrecord_pct_kfab_res_end_date", "isempty", ""]],
                        "AND",
                        ["custrecord_pct_kfab_wo", "noneof", "@NONE@"],
                        "AND",
                        ["custrecord_pct_kfab_emp", "anyof", user],
