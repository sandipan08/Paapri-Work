/**
 *              //////////     PMC QMS 2.1 | Solvent Testing Form Page    //////////
 * 
 *@author       Sandipan Sau
 *@NApiVersion  2.1
    ...
    function (file, render, search, log, redirect, record, format, email, runtime, url)
    {
        ...
        function onRequest(context)
        {
            ...
            if (request.method == 'GET')
            {
                ...
                else if (documentStatus == "Existing")
                {
                    var customrecord_pct_pp_qms_recordSearchObj = search.create({
                        ...
                        columns:
                            [
                                ...
                                search.createColumn({ name: "custrecord_pct_pp_flash_point", label: "Flash Point" }),
                    ...
                    for (var qms_index = 0; qms_index < qmsRecordCount; qms_index++)
                    {
                        ...
                        var flashPoint = qmsRecordResult[qms_index].getValue('custrecord_pct_pp_flash_point');
                    ...
                    log.debug({
                        title: 'PCT-QMS', details: "Product Name : " + productName + ", Date : " + entryDate + ", Batch No : " + batchNo + ", Party Name : " + partyName + ", SP Gravity : " + spGravity +
                            ", Vehicle No :" + vehicleNo + "Viscosity : " + viscosity + ",Temperature : " + temperature + ", Color : " + color + ", FlashPoint : " + flashPoint + ", PH Value : " + phValue + ", QC Approved : " + qcChecked + ", Report : " + report
                    });

                }
                // Load Login HTML Template
                var templateFile = file.load({ id: '../../HTML Files/Forms/pct_qms_solvent_testing_report_form.html' });
            ...
            else
            {
                ...
                var flashPoint = request.parameters.flashPoint;
                ...
                var solventFormObj = record.create({ type: "customrecord_pct_pp_qms_record", isDynamic: true });
                solventFormObj.setText({ fieldId: 'customform', text: "Solvent Testing Form" });
                if (documentType == "itemreceipt")
                {
                    solventFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Item Receipt" });
                }
                else if (documentType == "assemblybuild")
                {
                    solventFormObj.setText({ fieldId: 'custrecord_pct_pp_record_form', text: "Assembly Build" });
                }
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_document_number', value: documentNumber });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_date', value: entryDate });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_material_name_pdct_nme', value: productName });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_batch_no', value: batchNo });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_party_name', value: partyName });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_vehicles_no', value: vehicleNo });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_ph_value', value: phValue });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_viscosity', value: viscosity });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_temperature', value: temperature });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_colour', value: color });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_flash_point', value: flashPoint });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_sp_gravity', value: spGravity });
                solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_report', value: report });
                if (qcChecked === "on")
                {
                    solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: true });
                }
                else { solventFormObj.setValue({ fieldId: 'custrecord_pct_pp_qc_approved', value: false }); }
                var qmsRecordId = solventFormObj.save();
