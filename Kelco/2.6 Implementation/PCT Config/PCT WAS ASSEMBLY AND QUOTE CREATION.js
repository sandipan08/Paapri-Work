...
define(["N/record", "N/search", 'N/runtime'], function (record, search, runtime) {
    ...
    function onAction(scriptContext) {
        try {
            let purchaseVarianceAccount = 535;
            let unbuildVarianceAccount = 536;
            let productionQuantityVarianceAccount = 529;
            let productionPriceVarianceAccount = 533;
            ...
            let itemClass = configRecordObj.getValue({ fieldId: 'custrecord_pct_cpq_item_class' })
            ...
            let productFamily = configRecordObj.getValue({ fieldId: 'custrecord_pct_cpq_product_family' })
            let partNumber = configRecordObj.getValue({ fieldId: 'custrecord_pct_cpq_legacy_part_number' })
            ...
            let itemObj = record.create({
                ...
                fieldId: 'custitem_pct_kelco_old_part_number',
                value: partNumber
            }).setValue({
                fieldId: 'cseg_pct_kelco_ic',
                value: itemClass
            }).setValue({
                fieldId: 'class',
                value: productFamily
                ...
                fieldId: 'purchasepricevarianceacct',
                value: purchaseVarianceAccount
            }).setValue({
                fieldId: 'unbuildvarianceaccount',
                value: unbuildVarianceAccount
            }).setValue({
                fieldId: 'prodqtyvarianceacct',
                value: productionQuantityVarianceAccount
            }).setValue({
                fieldId: 'prodpricevarianceacct',
                value: productionPriceVarianceAccount
            }).setValue({
                fieldId: 'taxschedule',
                value: 1
            }).setValue({
                ...
                fieldId: 'custitem_pct_kelco_sellable',
                value: true
            }).setValue({
                fieldId: 'description',
            ...
            log.debug("PCT-Config", "Created Item : " + itemId)
    ...
    const createRouting = (routingDataObj) => {
        log.debug("PCT-Config", "In Create Routing Function")
        log.debug("PCT-Config", JSON.stringify(routingDataObj))
