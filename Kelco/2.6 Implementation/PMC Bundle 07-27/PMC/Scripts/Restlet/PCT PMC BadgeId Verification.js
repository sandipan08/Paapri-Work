...
define(['N/search', 'N/record', 'N/runtime'], function (search, record, runtime) {
    ...
    function _get(context) {
        ...
        if (employeeDetailsObj.isSuccess) {
            ...
            employeeDetailsObj.data['classes'] = classes;
        }
    ...
    const verifyEmployeeWithBadgeId = (badgeId) => {
        ...
        if (employeeCount > 0) {
            ...
            getEmployeeSearch.run().each(function (result) {
                ...
                employeeData['class'] = result.getValue({ name: "class" });
                employeeData['className'] = result.getText({ name: "class" });
                employeeData['userRole'] = runtime.getCurrentUser().role;
                employeeData['statusId'] = 1;
                return true;
            });
    ...
    const getClasses = () => {
        ...
        classificationSearchObj.run().each(function (result) {
            let res = {};
            res['name'] = result.getValue({ name: "name" })
            res['internalId'] = result.id;
            classArr.push(res);
