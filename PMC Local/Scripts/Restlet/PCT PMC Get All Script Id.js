...
fetchAllScriptId                                                Fetches restlet internal ids                                           Sandipan Sau 
/**********************************************************************************************************************************************
Update Log
...
define(['N/search'], function (search) {
    ...
    function _post(context) {
        ...
        log.debug("PCT-PMC", "Script Internal Id Obj : " + JSON.stringify(scriptInternalIdObj));
        return scriptInternalIdObj

    }
    // ----------------------------- Search to Get All Script Internal Id  ------------------------------
    const fetchAllScriptId = (filterArray, contextObj) => {
        log.debug("PCT-PMC", "Filter Array : " + filterArray + ", Context : " + JSON.stringify(contextObj));
        ...
        log.debug("PCT-PMC-fetchAllScriptId Function", "Script Count : " + searchResultCount);
        if (searchResultCount > 0) {
            scriptSearchObj.run().each(function (result) {
                let scriptInternalId = result.id;
                let scriptId = result.getValue({ name: "scriptid" });
                log.debug("PCT-PMC", "Script Internal Id : " + scriptInternalId + ", Script Id : " + scriptId)
