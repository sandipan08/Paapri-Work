...
define(['N/log', 'N/record', 'N/runtime', 'N/file', 'N/format', 'N/search', 'N/email', 'N/runtime'], function (log, record, runtime, file, format, search, email, runtime)
{
    function onAction(scriptContext)
    {
        ...
        try
        {
            ...
            // else
            // {
            //     var customer_id = searchCustomer(shopify_customerName, shopify_customerId, shopify_shippingCountry, shopify_customerMail, shopify_customerPhnNo);
            // }
            ...
            // if (shopify_shippingMethod.length != 0)
            // {
            //     var shipping_method_id = shipping_method(shopify_shippingMethod, shopify_weborder_load)
            //     salesOrderobj.setValue({ fieldId: 'shipmethod', value: shipping_method_id });
            // }
            if (shopify_shippingCost > 0)
            {
                salesOrderobj.setValue({ fieldId: 'shipmethod', value: 30 });
                shopify_weborder_load.setValue({ fieldId: 'custrecord_pct_swo_shippingmethod', value: "Standard Shipping" });

                // shopify_weborder_load.setValue({ fieldId: 'custrecord_pct_swo_error', value: "Error : Shipping Cost is greater than 0, you have to use a Shipping Method" });
            }
            ...
            else
            {
                ...
                for (item_index = 0; item_index < shopify_item_count; item_index++)
                {
                    ...
                    else
                    {
                        ...
                        // if (item_type != "Group")
                        // {
                        //     salesOrderobj.setCurrentSublistText({ sublistId: 'item', fieldId: 'units', text: item_unit });
                        //     //  salesOrderobj.setCurrentSublistValue({ sublistId: 'item', fieldId: 'units_display', value: item_unit });
                        // }
                ...
                else
                {
                    ...
                    if (soResultCount > 0)
                    {
                        for (var soIndex = 0; soIndex < soResultCount; soIndex++)
                        {
                            ...
                            var invoiceInternalNo = soResult[soIndex].getValue({
                                name: "internalid",
                                join: "applyingTransaction"
                            });
                        }
                ...
                if (item_fullfillment_id) // Item Fulfillmemt will be perform 1st then bill will be created
                {
                    //--------------------------------------------- Billing ----------------------------------------------
                    ...
                    if (shopify_orderRefund == true)
                    {
                        ...
                        soLoad.setValue({ fieldId: 'custbody_pct_moju_credit_memo', value: refund_id });
                        soLoad.save();
                    }
                }
            }
        ...
        // function searchCustomer(shopify_customerName, shopify_customerId, shopify_shippingCountry, shopify_customerMail, shopify_customerPhnNo)
        // {
        ...
        //                 // "AND",
        //                 // ["entityid", "is", shopify_customerId]
        //             ],
        //         columns:
        //             [
        //                 search.createColumn({ name: "internalid", label: "Internal ID" })
        ...
        //         //customer.setValue({ fieldId: 'entityid', value: shopify_customerId });
            ...
            var salesorderSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        ["internalid", "anyof", salesOrderInternalId],
                        "AND",
                        ["item.type", "noneof", "Discount", "Group"],
                        /*"AND", 
