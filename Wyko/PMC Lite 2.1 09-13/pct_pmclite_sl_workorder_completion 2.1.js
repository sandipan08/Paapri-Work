...
define(['N/file', 'N/render', 'N/search', 'N/log', 'N/redirect', 'N/url', 'N/record', 'N/format', 'N/error'],
    function (file, render, search, log, redirect, url, record, format, error) {
            ...
            if (_request.method == 'GET') {
                try {
                    ...
                    if (checkStatus != 'COMPLETE') {
                        ...
                        let lastOperationResponse = IsLastOperation(wordorderId)
                        log.debug("PCT-lastOperationResponse", JSON.stringify(lastOperationResponse))
                        if (lastOperationResponse.isSuccess && lastOperationResponse.data.internalid == operationInternalId) {
                            let itemTypeResponse = IsLotOrSerial(itemId)
                            log.debug("PCT-itemTypeResponse", JSON.stringify(itemTypeResponse))
                            if (itemTypeResponse.isSuccess && requestedQtyToComplete > 0 && itemTypeResponse.data) {
                                // Operation for Serialized Item
                                let lastCompletedQty = lastOperationResponse.data.lastCompletedQty;
                                log.debug("PCT-lastCompletedQty", lastCompletedQty)
                                for (let requestedQtyToCompleteIndex = 0; requestedQtyToCompleteIndex < requestedQtyToComplete; requestedQtyToCompleteIndex++) {
                                    ...
                                    log.debug("Select Line ", subRecordObject.selectNewLine({
                                        sublistId: 'inventoryassignment',
                                    }))
                                    subRecordObject.setCurrentSublistValue({
                                        sublistId: 'inventoryassignment',
                                        fieldId: 'receiptinventorynumber',
                                        value: `${tranid}-${lastCompletedQty + 1}`,//FormateDate("12/11/2019"),
                                    });
                                    subRecordObject.setCurrentSublistValue({
                                        sublistId: 'inventoryassignment',
                                        fieldId: 'quantity',
                                    ...
                                    lastCompletedQty = (lastCompletedQty + 1);
                                }
                            }
                            else {
                                // Operation for Lot Number Item
                                var subRecordObject = transformRecordObject.getSubrecord({
                                    fieldId: 'inventorydetail'
                                });
        ...
        function IsLastOperation(wordorderId) {
            ...
            let lastOperationObj = {}
            var manufacturingoperationtaskSearchObj = search.create({
                type: "manufacturingoperationtask",
                filters: [
                    ["status", "anyof", "PROGRESS", "NOTSTART"], "AND", ["workorder", "anyof", wordorderId]
            ...
            if (searchResultCount > 0) {
                manufacturingoperationtaskSearchObj.run().each(function (result) {
                    ...
                    lastOperationObj['lastCompletedQty'] = result.getValue({ name: "completedquantity", label: "Completed Quantity" }) == '' ? 0 : result.getValue({ name: "completedquantity", label: "Completed Quantity" });
                    lastOperationObj['internalid'] = result.getValue({ name: "internalid", label: "Internal ID", sort: search.Sort.DESC })

                    // return true;
                });
                log.debug("PCT-inside", JSON.stringify(lastOperationObj))
                return { 'isSuccess': true, 'data': lastOperationObj }
            } else {
                return { 'isSuccess': false, 'data': lastOperationObj }
            }
        ...
        function IsLotOrSerial(itemId) {
            ...
            var itemSearchObj = search.create({
                ...
                filters: [
                    ...
                    [["islotitem", "is", "T"], "OR",
                    ["isserialitem", "is", "T"]]]],
                columns: [search.createColumn({ name: "itemid", label: "Name" }),
            ...
            if (searchResultCount > 0) {
                ...
                itemSearchObj.run().each(function (result) {
                    ...
                    let islotitem = result.getValue({ name: "islotitem", label: "Is Lot Numbered Item" })
                    let isserialitem = result.getValue({ name: "isserialitem", label: "Is Serialized Item" })
                    log.debug({
                        title: "PCT",
                        details: "islotitem : " + islotitem + ", isserialitem : " + isserialitem
                    })
                    if (isserialitem) { response = true }
                    // return true;
                });
        ...
        function calSumofAction(userId, operationId, now) {
            ...
            customrecord_pct_pmc_tran_k_fabSearchObj.run().each(function (result) {
                ...
                totalTime = parseInt(duration) - parseInt(downTimeDuration);
