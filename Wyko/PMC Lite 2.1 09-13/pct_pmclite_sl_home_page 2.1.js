...
define(['N/file', 'N/render', 'N/search', 'N/log', 'N/redirect', 'N/url', 'N/record', 'N/format'],
    function (file, render, search, log, redirect, url, record, format) {
        ...
        function PageRendererByMode(userId, userName, todayQuantity, loginRecordId, startRecordId, logoutPageUrl, breakPageUrl, homePageUrl, woCompletionUrl, ticketNumber, mode, dataObj) {
            ...
            var downTimeReason = getDownReason('customrecord_pct_pmc_dwn_reason');
            var downTimeCategory = getDownReason('customrecord_pct_pmc_dwn_cat');
            let getWorkInstruction = getInstruction(dataObj.workOrderInternalId, dataObj.sequence)
            log.debug("PCT-getWorInstruction", getWorkInstruction)
            log.debug({
            ...
            var dataSource = {
                ...
                workorder: dataObj.workorder.split('#')[1],//dataObj.workorder,
                workInstruction: getWorkInstruction.toString(),
                item: dataObj.item,
        ...
        function getDownReason(recordId) {
            ...
            dataArr.push({ 'name': "--Select--", "internalid": -1 });
     ...
     */
        ...
        function GetManufacturingOperationDetails(internalId, sequence) {
            ...
            if (searchResultCount > 0) {
                mfgOpTaskSearchObj.run().each(function (result) {
                    ...
                    searchResults.workorder = result.getValue({ name: 'workorder' });//result.getValue({ name: 'workorder' });
                    ...
                    searchResults.workOrderInternalId = result.getValue({
                        name: "internalid",
                        join: "workOrder",
                        label: "Internal ID"
                    });
                    ...
                    searchResults.predecessorCompletedquantity = CheckNull(predecessorCompletedquantity);
        ...
        */
        ...
        const getInstruction = (workOrderId, operationSequence) => {
            log.debug("PCT-getInstruction", workOrderId + " " + operationSequence)
            ...
            customrecord_pct_pmc_instructionSearchObj.run().each(function (result) {
                instructionArray.push(result.getValue({ name: "custrecord_pct_pmc_ins_work_ins", label: "Instruction" }))
                return true;
            });
